...
$vamTemplate->assign('PRODUCTS_NAME', $reviews['articles_name']);
...
$products_image = DIR_FS_CATALOG.DIR_WS_IMAGES.'articles/'.$reviews['articles_image'];
if (!is_file($products_image)) $products_image = DIR_WS_THUMBNAIL_IMAGES.'../noimage.gif';
$image = vam_image($products_image, $reviews['articles_name'], '', '', 'hspace="5" vspace="5"');
...
// set cache ID
 if (!CacheCheck()) {
	...
	$main_content = $vamTemplate->fetch(CURRENT_TEMPLATE.'/module/article_reviews_info.html');
} else {
	...
	$main_content = $vamTemplate->fetch(CURRENT_TEMPLATE.'/module/article_reviews_info.html', $cache_id);
}
...
$template = (file_exists('templates/'.CURRENT_TEMPLATE.'/'.FILENAME_ARTICLE_REVIEWS_INFO.'.html') ? CURRENT_TEMPLATE.'/'.FILENAME_ARTICLE_REVIEWS_INFO.'.html' : CURRENT_TEMPLATE.'/index.html');
