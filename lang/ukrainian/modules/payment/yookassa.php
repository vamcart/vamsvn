<?php
/* -----------------------------------------------------------------------------------------
   $Id: yookassa.php 2588 2010/04/13 13:24:46 oleg_vamsoft $
...
// class constructor
    public function __construct() {
      ...
      $this->code = 'yookassa';
      $this->title = MODULE_PAYMENT_YOOKASSA_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_YOOKASSA_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_YOOKASSA_TEXT_ADMIN_DESCRIPTION;
      $this->icon = DIR_WS_ICONS . 'yookassa.png';
      $this->sort_order = MODULE_PAYMENT_YOOKASSA_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_YOOKASSA_STATUS == 'True') ? true : false);
...
// class methods
    public function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_YOOKASSA_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_YOOKASSA_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
	...
	function selection() {
      ...
      if (isset($_SESSION['cart_yookassa_id'])) {
        $order_id = substr($_SESSION['cart_yookassa_id'], strpos($_SESSION['cart_yookassa_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_yookassa_id']);
        }
      }
    ...
    public function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_yookassa_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($_SESSION['cart_yookassa_id'], 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_yookassa_id'] = $cartID . '-' . $insert_id;
        }
...
// Выписываем qiwi счёт для покупателя
		...
		//if (defined('MODULE_PAYMENT_YOOKASSA_SEND_CHECK') && constant('MODULE_PAYMENT_YOOKASSA_SEND_CHECK') == 'True') {
...
include_once(DIR_FS_CATALOG.'vendor/yandex-checkout-sdk-php/lib/autoload.php');
    ...
    vam_redirect($confirmation->getConfirmationUrl());

        }

      }

      return array('title' => MODULE_PAYMENT_YOOKASSA_TEXT_DESCRIPTION);
    }
	...
	function process_button() {
      ...
      $process_button_string = vam_draw_hidden_field('kvit_name', $this->name) .
		...
		//vam_redirect('https://w.qiwi.com/order/external/main.action?shop='.MODULE_PAYMENT_YOOKASSA_ID.'&transaction='.substr($_SESSION['cart_yookassa_id'], strpos($_SESSION['cart_yookassa_id'], '-')+1).'&successUrl='.vam_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL').'&failUrl='.vam_href_link(FILENAME_CHECKOUT, '', 'SSL'));
...
// unregister session variables used during checkout
    ...
    public function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_YOOKASSA_STATUS'");
    ...
    public function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_YOOKASSA_STATUS', 'True', '6', '1', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_YOOKASSA_ALLOWED', '', '6', '2', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_YOOKASSA_SHOP_ID', '', '6', '3', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_YOOKASSA_SECRET_KEY', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_YOOKASSA_TEST', 'test', '6', '5', 'vam_cfg_select_option(array(\'test\', \'production\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_YOOKASSA_SORT_ORDER', '0', '6', '6', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_YOOKASSA_ZONE', '0', '6', '7', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_YOOKASSA_ORDER_STATUS_ID', '0', '6', '9', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_YOOKASSA_PAYMENT_TYPE', '', '6', '10', 'vam_cfg_select_option(array(\'\', \'PC\', \'AC\', \'MC\', \'GP\', \'WM\', \'SB\', \'MP\', \'AB\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_YOOKASSA_SEND_CHECK', 'False', '6', '11', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
    }
    ...
    public function keys() {
      return array('MODULE_PAYMENT_YOOKASSA_STATUS', 'MODULE_PAYMENT_YOOKASSA_ALLOWED', 'MODULE_PAYMENT_YOOKASSA_SHOP_ID', 'MODULE_PAYMENT_YOOKASSA_SECRET_KEY', 'MODULE_PAYMENT_YOOKASSA_TEST', 'MODULE_PAYMENT_YOOKASSA_SORT_ORDER', 'MODULE_PAYMENT_YOOKASSA_ZONE', 'MODULE_PAYMENT_YOOKASSA_ORDER_STATUS_ID', 'MODULE_PAYMENT_YOOKASSA_TEST', 'MODULE_PAYMENT_YOOKASSA_PAYMENT_TYPE', 'MODULE_PAYMENT_YOOKASSA_SEND_CHECK');
    }
