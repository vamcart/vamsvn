...
$orders_query = "select
  ...
  foreach ($products AS $product_id_in_cart) {
	...
	function getWishlistButton($id, $name) {
		...
		} elseif (AJAX_WISHLIST == 'false' && vam_has_product_attributes($id)) {
		...
		$link = '<a class="btn btn-add-to-cart btn-block" href="'.vam_href_link(FILENAME_WISHLIST, 'action=wishlist_now&BUYproducts_id='.$id, 'NONSSL').'"><i class="far fa-heart"></i> '.IMAGE_BUTTON_IN_WISHLIST.'</a>';
		}
