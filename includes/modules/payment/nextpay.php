<?php
/* -----------------------------------------------------------------------------------------
   $Id: nextpay.php 2588 2010/04/13 13:24:46 oleg_vamsoft $
...
// class constructor
    public function __construct() {
      ...
      $this->code = 'nextpay';
      $this->title = MODULE_PAYMENT_NEXTPAY_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_NEXTPAY_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_NEXTPAY_TEXT_ADMIN_DESCRIPTION;
      $this->icon = DIR_WS_ICONS . 'nextpay.png';
      $this->sort_order = MODULE_PAYMENT_NEXTPAY_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_NEXTPAY_STATUS == 'True') ? true : false);
...
// class methods
    public function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_NEXTPAY_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_NEXTPAY_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_nextpay_id'])) {
        $order_id = substr($_SESSION['cart_nextpay_id'], strpos($_SESSION['cart_nextpay_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_nextpay_id']);
        }
      }
    ...
    public function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_nextpay_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($_SESSION['cart_nextpay_id'], 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_nextpay_id'] = $cartID . '-' . $insert_id;
        }
...
// Переход на nextpay
		...
		vam_redirect('https://www.nextpay.ru/buy/index.php?command=show_product_form_ext&product_id='.MODULE_PAYMENT_NEXTPAY_PRODUCT_ID.'&seller_ext_order_id='.substr($_SESSION['cart_nextpay_id'], strpos($_SESSION['cart_nextpay_id'], '-')+1).'&ext_order_cost='.$order->info['total'].'');

      return array('title' => MODULE_PAYMENT_NEXTPAY_TEXT_DESCRIPTION);
    }
...
// unregister session variables used during checkout
    ...
    public function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_NEXTPAY_STATUS'");
    ...
    public function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_NEXTPAY_STATUS', 'True', '6', '1', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NEXTPAY_ALLOWED', '', '6', '2', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NEXTPAY_PRODUCT_ID', '', '6', '3', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NEXTPAY_SECRET_KEY', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NEXTPAY_SORT_ORDER', '0', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_NEXTPAY_ZONE', '0', '6', '6', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_NEXTPAY_ORDER_STATUS_ID', '0', '6', '7', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
    ...
    public function keys() {
      return array('MODULE_PAYMENT_NEXTPAY_STATUS', 'MODULE_PAYMENT_NEXTPAY_ALLOWED', 'MODULE_PAYMENT_NEXTPAY_PRODUCT_ID', 'MODULE_PAYMENT_NEXTPAY_SECRET_KEY', 'MODULE_PAYMENT_NEXTPAY_SORT_ORDER', 'MODULE_PAYMENT_NEXTPAY_ZONE', 'MODULE_PAYMENT_NEXTPAY_ORDER_STATUS_ID');
    }
