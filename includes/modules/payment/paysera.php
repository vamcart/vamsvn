<?php
/* -----------------------------------------------------------------------------------------
   $Id: paysera.php 998 2007/02/07 13:24:46 VaM $
...
// class constructor
    function __construct() {
      ...
      $this->public_title = MODULE_PAYMENT_PAYSERA_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_PAYSERA_TEXT_ADMIN_DESCRIPTION;
      $this->icon = DIR_WS_ICONS . 'paysera.png';
      $this->sort_order = MODULE_PAYMENT_PAYSERA_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_PAYSERA_STATUS == 'True') ? true : false);

            $libwebtopay_dir = substr(__FILE__, 0, -4) .'/paysera/WebToPay.php';
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_PAYSERA_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_PAYSERA_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_PAYSERA_PROJECT'])) {
        $order_id = substr($_SESSION['cart_PAYSERA_PROJECT'], strpos($_SESSION['cart_PAYSERA_PROJECT'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_PAYSERA_PROJECT']);
        }
      }
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_PAYSERA_PROJECT'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($_SESSION['cartID'] != substr($_SESSION['cart_PAYSERA_PROJECT'], 0, strlen($_SESSION['cartID']))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_PAYSERA_PROJECT'] = $_SESSION['cartID'] . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_PAYSERA_TEXT_DESCRIPTION);
    }
    
		function process_button() {
			global $order, $currencies, $currency, $customer_id;

            $orderid = substr($_SESSION['cart_PAYSERA_PROJECT'], strpos($_SESSION['cart_PAYSERA_PROJECT'], '-')+1);
			...
			$process_button_string = '';
	            ...
	            try {
	                $request = WebToPay::buildRequest(array(
	                        ...
	                        'accepturl'     => vam_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL'),
	                        'cancelurl'     => vam_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL'),
	                        'callbackurl'   => vam_href_link('ext/modules/payment/paysera/callback.php', '' , 'SSL', true, true, true),
            ...
            if($request) {
	            foreach( $request as $field=>$value) {
	                $process_button_string .= vam_draw_hidden_field($field, $value);
	            }
            } else {
            	$process_button_string .= vam_draw_hidden_field('cancelurl', vam_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL'));;
            }
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_PAYSERA_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_PAYSERA_STATUS', 'True', '6', '3', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PAYSERA_ALLOWED', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PAYSERA_PROJECT', '', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_PAYSERA_TESTING', 'No', '6', '3', 'vam_cfg_select_option(array(\'No\', \'Yes\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PAYSERA_SORT_ORDER', '0', '6', '7', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_PAYSERA_ZONE', '0', '6', '8', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PAYSERA_SIGNATURE', '', '6', '9', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_PAYSERA_ORDER_STATUS_ID', '0', '6', '10', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_PAYSERA_STATUS', 'MODULE_PAYMENT_PAYSERA_ALLOWED', 'MODULE_PAYMENT_PAYSERA_PROJECT', 'MODULE_PAYMENT_PAYSERA_TESTING', 'MODULE_PAYMENT_PAYSERA_SORT_ORDER', 'MODULE_PAYMENT_PAYSERA_ZONE', 'MODULE_PAYMENT_PAYSERA_SIGNATURE', 'MODULE_PAYMENT_PAYSERA_ORDER_STATUS_ID');
    }
