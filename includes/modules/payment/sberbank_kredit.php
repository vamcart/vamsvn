<?php
/*------------------------------------------------------------------------------
  $Id: sberbank_kredit.php 1406 2017-12-15 00:00:08 dependab1e $
   ---------------------------------------------------------------------------------------*/

  class sberbank_kredit {
...
// class constructor
    function __construct() {
      ...
      $this->code = 'sberbank_kredit';
      $this->title = MODULE_PAYMENT_SBERBANK_KREDIT_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_SBERBANK_KREDIT_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_SBERBANK_KREDIT_TEXT_ADMIN_DESCRIPTION;
      $this->icon = DIR_WS_ICONS . 'sberbank.png';
      $this->sort_order = MODULE_PAYMENT_SBERBANK_KREDIT_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_SBERBANK_KREDIT_STATUS == 'True') ? true : false);
      $this->form_action_url = ''; // set after first authorisation step
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_SBERBANK_KREDIT_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_SBERBANK_KREDIT_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
		///Was ist das ?
      if (isset($_SESSION['cart_sberbank_kredit_id'])) {
        $order_id = substr($_SESSION['cart_sberbank_kredit_id'], strpos($_SESSION['cart_sberbank_kredit_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_sberbank_kredit_id']);
        }
      }
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_sberbank_kredit_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($_SESSION['cart_sberbank_kredit_id'], 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_sberbank_kredit_id'] = $cartID . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_SBERBANK_KREDIT_TEXT_DESCRIPTION);
    }
    ...
    function process_button() {
        ...
        include_once(DIR_FS_CATALOG.'vendor/sberbank/rbsCreditLibrary.php');
        $this->rbsCreditLibrary = new rbsCreditLibrary();
        $this->rbsCreditLibrary->login = MODULE_PAYMENT_SBERBANK_KREDIT_UNAME;
        $this->rbsCreditLibrary->password = MODULE_PAYMENT_SBERBANK_KREDIT_PASS;
        $this->rbsCreditLibrary->mode = MODULE_PAYMENT_SBERBANK_KREDIT_MODE;
        $this->rbsCreditLibrary->currency = 643;
        $this->rbsCreditLibrary->taxSystem = 0;
        $this->rbsCreditLibrary->language = 'ru';
        $this->rbsCreditLibrary->logging = 0;

        $this->rbsCreditLibrary->returnUrl = vam_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL');
        $this->rbsCreditLibrary->failUrl = vam_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL');
        

        $order_info = $order;
        $order_number = (int)substr($_SESSION['cart_sberbank_kredit_id'], strpos($_SESSION['cart_sberbank_kredit_id'], '-')+1);
        $amount = round($order->info['total'] * 100);

        $return_url = vam_href_link('sberbank.php', '', 'SSL');
        ...
        foreach ($order->products as $product) {
            ...
            $product_taxSum = 0;
            $product_amount = ( $product['price'] + $product_taxSum ) * $product['qty'];
        ...
        // DELIVERY
        if ($order->info['total'] > $order->info['subtotal']) {

            $delivery['positionId'] = 'delivery';
            $delivery['name'] = 'Доставка';
            $delivery['itemAmount'] = round($order->info['total'] - $order->info['subtotal']) * 100;
            $delivery['quantity']['value'] = 1;
            //todo fix piece
            $delivery['quantity']['measure'] = 'piece';
            $delivery['itemCode'] = 'shipping';
            $delivery['tax']['taxType'] = 0;
            $delivery['tax']['taxSum'] = 0;
            $delivery['itemPrice'] = round($order->info['total'] - $order->info['subtotal']) * 100;
        ...
        $customer_phone = preg_replace("/(\W*)/", "", $order->customer['telephone']);
        ...
        $url = $this->rbsCreditLibrary->register_order($order_number, $amount, $return_url, $customer_phone, $orderBundle);
        
        //echo var_dump($url);

		$this->form_action_url = $url['formUrl'];
		if ($url['formUrl'] != '') {
		$process_button_string = '<script>document.location.href="'.$url['formUrl'].'";document.checkoutUrl.method="get"</script>';
		}
...
// unregister session variables used during checkout
    ...
    function check() {
      ...
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_SBERBANK_KREDIT_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_SBERBANK_KREDIT_STATUS', 'True', '6', '3', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_SBERBANK_KREDIT_ALLOWED', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_SBERBANK_KREDIT_UNAME', '', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_SBERBANK_KREDIT_PASS', '', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_SBERBANK_KREDIT_MODE', 'test', '6', '5', 'vam_cfg_select_option(array(\'test\', \'prod\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_SBERBANK_KREDIT_SECRET', '', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_SBERBANK_KREDIT_SORT_ORDER', '0', '6', '7', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_SBERBANK_KREDIT_ZONE', '0', '6', '8', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_SBERBANK_KREDIT_ORDER_STATUS_ID', '0', '6', '10', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_SBERBANK_KREDIT_STATUS', 'MODULE_PAYMENT_SBERBANK_KREDIT_ALLOWED', 'MODULE_PAYMENT_SBERBANK_KREDIT_UNAME', 'MODULE_PAYMENT_SBERBANK_KREDIT_PASS', 'MODULE_PAYMENT_SBERBANK_KREDIT_MODE', 'MODULE_PAYMENT_SBERBANK_KREDIT_SORT_ORDER', 'MODULE_PAYMENT_SBERBANK_KREDIT_ZONE', 'MODULE_PAYMENT_SBERBANK_KREDIT_ORDER_STATUS_ID', 'MODULE_PAYMENT_SBERBANK_KREDIT_SECRET');
    }
    ...
    public function _registerOrder() {
        ...
        $return_url = $this->url->link('extension/payment/rbs_installment_callback/');
