<?php
/* -----------------------------------------------------------------------------------------
   $Id: stripe.php 998 2007/02/07 13:24:46 VaM $
...
// class constructor
    function __construct() {
      ...
      $this->description = MODULE_PAYMENT_STRIPE_TEXT_ADMIN_DESCRIPTION;
      $this->icon = DIR_WS_ICONS . 'stripe.png';
      $this->sort_order = MODULE_PAYMENT_STRIPE_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_STRIPE_STATUS == 'True') ? true : false);

      $this->form_action_url = vam_href_link('stripe.php', '', 'SSL');
    }
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_STRIPE_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_STRIPE_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_stripe_id'])) {
        $order_id = substr($_SESSION['cart_stripe_id'], strpos($_SESSION['cart_stripe_id'], '-')+1);
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_stripe_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($_SESSION['cartID'] != substr($_SESSION['cart_stripe_id'], 0, strlen($_SESSION['cartID']))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_stripe_id'] = $_SESSION['cartID'] . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_STRIPE_TEXT_DESCRIPTION);
    }
...
\Stripe\Stripe::setApiKey($stripe['secret_key']);
      ...
      $process_button_string = vam_draw_hidden_field('order_id', substr($_SESSION['cart_stripe_id'], strpos($_SESSION['cart_stripe_id'], '-')+1));
...
<script>
  Stripe.setPublishableKey("'.MODULE_PAYMENT_STRIPE_PUBLIC_KEY.'");
</script>
...
function stripeResponseHandler(status, response) {
  ...
  if (response.error) { // Problem!
    ...
    $form.get(0).submit();
		...
		<div class="error"><span class="payment-errors"></span></div>
		<div id="stripe">

<fieldset class="form">

<p>Card Number: <input type="text" class="form-control" data-stripe="number" /></p>
...
<p>CVC: <input type="text" class="form-control" maxlength="4" data-stripe="cvc" /></p>
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_STRIPE_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_STRIPE_STATUS', 'True', '6', '3', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_STRIPE_ALLOWED', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_STRIPE_PUBLIC_KEY', '', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_STRIPE_SORT_ORDER', '0', '6', '6', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_STRIPE_ZONE', '0', '6', '7', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_STRIPE_SECRET_KEY', '', '6', '8', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_STRIPE_ORDER_STATUS_ID', '0', '6', '9', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_STRIPE_STATUS', 'MODULE_PAYMENT_STRIPE_ALLOWED', 'MODULE_PAYMENT_STRIPE_PUBLIC_KEY', 'MODULE_PAYMENT_STRIPE_SORT_ORDER', 'MODULE_PAYMENT_STRIPE_ZONE', 'MODULE_PAYMENT_STRIPE_SECRET_KEY', 'MODULE_PAYMENT_STRIPE_ORDER_STATUS_ID');
    }
