...
// class constructor
    function __construct() {
      ...
      $this->code = 'rficb';
      $this->title = MODULE_PAYMENT_RFICB_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_RFICB_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_RFICB_TEXT_ADMIN_DESCRIPTION;
      $this->icon = DIR_WS_ICONS . 'rficb.png';
      $this->sort_order = MODULE_PAYMENT_RFICB_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_RFICB_STATUS == 'True') ? true : false);

        $this->form_action_url = 'https://partner.rficb.ru/a1lite/input/';
    }
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_RFICB_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_RFICB_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_rficb_id'])) {
        $order_id = substr($_SESSION['cart_rficb_id'], strpos($_SESSION['cart_rficb_id'], '-')+1);
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_rficb_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($_SESSION['cart_rficb_id'], 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
if ($_SERVER["HTTP_X_FORWARDED_FOR"]) {
	...
	$customers_ip = $_SERVER["REMOTE_ADDR"];
          ...
          $_SESSION['cart_rficb_id'] = $cartID . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_RFICB_TEXT_DESCRIPTION);
    }
    ...
    function process_button() {
      ...
      $process_button_string = '';
                               ...
                               $order_id = substr($_SESSION['cart_rficb_id'], strpos($_SESSION['cart_rficb_id'], '-')+1);
                               $description = MODULE_PAYMENT_RFICB_ORDER_DESCRIPTION.' '.$order_id.' '.$_SERVER['SERVER_NAME'];

      $process_button_string = vam_draw_hidden_field('key', MODULE_PAYMENT_RFICB_KEY) .
                               vam_draw_hidden_field('cost', $order_sum) .
                               vam_draw_hidden_field('name', $description) .
                               vam_draw_hidden_field('default_email', $order->customer['email_address']) .
                               vam_draw_hidden_field('order_id', $order_id);
                               vam_draw_hidden_field('comment', $order_id);
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_RFICB_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_RFICB_STATUS', 'True', '6', '3', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_RFICB_ALLOWED', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_RFICB_KEY', '', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_RFICB_SECRET_KEY', '', '6', '6', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_RFICB_SORT_ORDER', '0', '6', '7', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_RFICB_ZONE', '0', '6', '8', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_RFICB_ORDER_STATUS_ID', '0', '6', '10', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_RFICB_STATUS', 'MODULE_PAYMENT_RFICB_ALLOWED', 'MODULE_PAYMENT_RFICB_KEY', 'MODULE_PAYMENT_RFICB_SECRET_KEY', 'MODULE_PAYMENT_RFICB_SORT_ORDER', 'MODULE_PAYMENT_RFICB_ZONE', 'MODULE_PAYMENT_RFICB_ORDER_STATUS_ID');
    }
