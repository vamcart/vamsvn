...
class kvitancia {
	...
	function update_status() {
		...
		if (($this->enabled == true) && ((int) MODULE_PAYMENT_KVITANCIA_ZONE > 0)) {
			$check_flag = false;
			$check_query = vam_db_query("select zone_id from ".TABLE_ZONES_TO_GEO_ZONES." where geo_zone_id = '".MODULE_PAYMENT_KVITANCIA_ZONE."' and zone_country_id = '".$order->delivery['country']['id']."' order by zone_id");
                         ...
                         'description'=>$this->info,
      	                 'fields' => array(array('title' => '<div id="kvitancia">'.MODULE_PAYMENT_KVITANCIA_NAME_TITLE,
      	                                   ...
      	                                   array('title' => MODULE_PAYMENT_KVITANCIA_NAME,
      	                                         'field' => vam_draw_input_field('kvit_name', $order->customer['firstname'] . ' ' . $order->customer['lastname'], 'class="form-control"')),
      	                                   array('title' => MODULE_PAYMENT_KVITANCIA_ADDRESS,
      	                                         'field' => vam_draw_input_field('kvit_address',$order->customer['city'] . ', ' . $order->customer['street_address'], 'class="form-control"') . MODULE_PAYMENT_KVITANCIA_ADDRESS_HELP.'</div>'),
      	                                   ));
