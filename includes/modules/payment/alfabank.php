<?php
/* -----------------------------------------------------------------------------------------
   $Id: alfabank.php 998 2007/02/07 13:24:46 VaM $
...
// class constructor
    function __construct() {
      ...
      $this->code = 'alfabank';
      $this->title = MODULE_PAYMENT_ALFABANK_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_ALFABANK_TEXT_PUBLIC_TITLE;
      $this->icon = DIR_WS_ICONS . 'alfabank.png';
      $this->description = MODULE_PAYMENT_ALFABANK_TEXT_ADMIN_DESCRIPTION;
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_ALFABANK_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_ALFABANK_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_alfabank_id'])) {
        $order_id = substr($_SESSION['cart_alfabank_id'], strpos($_SESSION['cart_alfabank_id'], '-')+1);
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_alfabank_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($_SESSION['cart_alfabank_id'], 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_alfabank_id'] = $cartID . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_ALFABANK_TEXT_DESCRIPTION);
    }
...
//echo var_dump($_responce);        
        ...
        if (isset($_responce['errorCode'])) {
            echo 'Ошибка #' . $_responce['errorCode'] . ': ' . $_responce['errorMessage'];
        }else{
            //echo '<html>' . "\n";
            //echo '<head>' . "\n";
...
// Переход на яндекс деньги
		...
		vam_redirect($_responce['formUrl']);
		die();
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_ALFABANK_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_ALFABANK_STATUS', 'True', '6', '1', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ALFABANK_ALLOWED', '', '6', '2', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ALFABANK_API_LOGIN', '', '6', '3', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ALFABANK_API_PASS', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ALFABANK_SORT_ORDER', '0', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_ALFABANK_ZONE', '0', '6', '6', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_ALFABANK_ORDER_STATUS_ID', '0', '6', '7', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_ALFABANK_TEST', 'test', '6', '8', 'vam_cfg_select_option(array(\'test\', \'production\'), ', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_ALFABANK_STATUS', 'MODULE_PAYMENT_ALFABANK_ALLOWED', 'MODULE_PAYMENT_ALFABANK_API_LOGIN', 'MODULE_PAYMENT_ALFABANK_API_PASS', 'MODULE_PAYMENT_ALFABANK_SORT_ORDER', 'MODULE_PAYMENT_ALFABANK_ZONE', 'MODULE_PAYMENT_ALFABANK_ORDER_STATUS_ID', 'MODULE_PAYMENT_ALFABANK_TEST');
    }
