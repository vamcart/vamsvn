...
class deltakey {
	...
	function __construct() {
		...
		$this->public_title = MODULE_PAYMENT_DELTAKEY_MERCHANT_TEXT_PUBLIC_TITLE;
		$this->description = MODULE_PAYMENT_DELTAKEY_MERCHANT_TEXT_ADMIN_DESCRIPTION;
		$this->icon = DIR_WS_ICONS . 'deltakey.png';
      $this->sort_order = MODULE_PAYMENT_DELTAKEY_MERCHANT_SORT_ORDER;
		$this->enabled = ((MODULE_PAYMENT_DELTAKEY_MERCHANT_STATUS == 'True') ? true : false);
		$this->form_action_url = 'https://merchant.deltakey.net/index.py';
	}
...
// class methods
	...
	function selection() {
		if (isset($_SESSION['cart_deltakey_id'])) {
			$order_id = substr($_SESSION['cart_deltakey_id'], strpos($_SESSION['cart_deltakey_id'], '-')+1);
			...
			if (vam_db_num_rows($check_query) < 1) {
				...
				unset($_SESSION['cart_deltakey_id']);
			}
		}
	...
	function confirmation() {
		...
		if (isset($_SESSION['cartID'])) {
			...
			if (isset($_SESSION['cart_deltakey_id'])) {
					...
					if ( ($curr['currency'] != $order->info['currency']) || ($_SESSION['cartID'] != substr($_SESSION['cart_deltakey_id'], 0, strlen($_SESSION['cartID']))) ) {
						$check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
						if (vam_db_num_rows($check_query) < 1) {
							vam_db_query('delete from ' . TABLE_ORDERS . ' where orders_id = "' . (int)$order_id . '"');
							vam_db_query('delete from ' . TABLE_ORDERS_TOTAL . ' where orders_id = "' . (int)$order_id . '"');
			...
			if ($insert_order == true) {
				...
				$_SESSION['cart_deltakey_id'] = $_SESSION['cartID'] . '-' . $insert_id;
			}
		}
		return array('title' => MODULE_PAYMENT_DELTAKEY_MERCHANT_TEXT_DESCRIPTION);
	}
	function process_button() {
		global $customer_id, $order, $sendto, $vamPrice, $currencies, $shipping;
		$process_button_string = '';

		$shop_id = MODULE_PAYMENT_DELTAKEY_MERCHANT_ID;
		$shop_key = MODULE_PAYMENT_DELTAKEY_MERCHANT_KEY;
		$shop_skey = MODULE_PAYMENT_DELTAKEY_MERCHANT_SECRET_KEY;
		$id= substr($_SESSION['cart_deltakey_id'], strpos($_SESSION['cart_deltakey_id'], '-')+1);
		$price = $order->info['total'];

		$param = $id.$shop_id.$shop_key.'1'.$price.'Order '.$id;
		$sign = $this->hmac($shop_skey, $param);
		
		$process_button_string = vam_draw_hidden_field('ext_transact', $id).
		vam_draw_hidden_field('keyt_shop', $shop_key).
		vam_draw_hidden_field('num_shop', $shop_id).
		vam_draw_hidden_field('comment',"Order ".$id).
		vam_draw_hidden_field('identified',"1").
		vam_draw_hidden_field('sign',$sign).
		vam_draw_hidden_field('sum',$price);
	...
	function check() {
		if (!isset($this->_check)) {
			$check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_DELTAKEY_MERCHANT_STATUS'");
			$this->_check = vam_db_num_rows($check_query);
		}
		return $this->_check;
	}
	function install() {
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_DELTAKEY_MERCHANT_STATUS', 'True', '6', '3', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_DELTAKEY_ALLOWED', '', '6', '4', now())");
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_DELTAKEY_MERCHANT_ID', '', '6', '5', now())");
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_DELTAKEY_MERCHANT_KEY', '', '6', '6', now())");
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_DELTAKEY_MERCHANT_SECRET_KEY', '', '6', '9', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_DELTAKEY_MERCHANT_SORT_ORDER', '0', '6', '7', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_DELTAKEY_MERCHANT_ZONE', '0', '6', '8', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_DELTAKEY_MERCHANT_ORDER_STATUS_ID', '0', '6', '10', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
	}
	function remove() {
		vam_db_query("delete from " . TABLE_CONFIGURATION . " where configuration_key in ('" . implode("', '", $this->keys()) . "')");
	}
	function keys() {
		return array('MODULE_PAYMENT_DELTAKEY_MERCHANT_STATUS', 'MODULE_PAYMENT_DELTAKEY_ALLOWED', 'MODULE_PAYMENT_DELTAKEY_MERCHANT_ID', 'MODULE_PAYMENT_DELTAKEY_MERCHANT_KEY', 'MODULE_PAYMENT_DELTAKEY_MERCHANT_SECRET_KEY', 'MODULE_PAYMENT_DELTAKEY_MERCHANT_SORT_ORDER', 'MODULE_PAYMENT_DELTAKEY_MERCHANT_ZONE', 'MODULE_PAYMENT_DELTAKEY_MERCHANT_ORDER_STATUS_ID');
	}
}
?>