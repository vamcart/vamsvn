<?php
/* -----------------------------------------------------------------------------------------
   $Id: psbank.php 998 2007/02/07 13:24:46 VaM $
...
// class constructor
    function __construct() {
      ...
      $this->code = 'psbank';
      $this->title = MODULE_PAYMENT_PSBANK_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_PSBANK_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_PSBANK_TEXT_ADMIN_DESCRIPTION;
      $this->icon = DIR_WS_ICONS . 'psbank.png';
      $this->sort_order = MODULE_PAYMENT_PSBANK_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_PSBANK_STATUS == 'True') ? true : false);

      ((MODULE_PAYMENT_PSBANK_TEST == 'test') ? $this->form_action_url = 'https://test.3ds.payment.ru/cgi-bin/cgi_link' : $this->form_action_url = 'https://3ds.payment.ru/cgi-bin/cgi_link');
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_PSBANK_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_PSBANK_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_psbank_id'])) {
        $order_id = substr($_SESSION['cart_psbank_id'], strpos($_SESSION['cart_psbank_id'], '-')+1);
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_psbank_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($_SESSION['cartID'] != substr($_SESSION['cart_psbank_id'], 0, strlen($_SESSION['cartID']))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_psbank_id'] = $_SESSION['cartID'] . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_PSBANK_TEXT_DESCRIPTION);
    }
    ...
    function process_button() {
      ...
      $order_sum = $order->info['total'];
		...
		$data = array(
			...
			'ORDER' => 1000000 + substr($_SESSION['cart_psbank_id'], strpos($_SESSION['cart_psbank_id'], '-')+1),
			'MERCH_NAME' => STORE_NAME,
			'MERCHANT' => MODULE_PAYMENT_PSBANK_MERCHANT,
			'TERMINAL' => MODULE_PAYMENT_PSBANK_TERMINAL,
			'EMAIL' => STORE_OWNER_EMAIL_ADDRESS,
			'TRTYPE' => '1',
			'TIMESTAMP' => $timestamp,
			// Случайное число неизвестного назначения
			'NONCE' => substr(md5(rand(1,9999).microtime()), 0, 16),
			'BACKREF' => vam_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL')
		);
		...
		foreach($data as $k=>$v)
		{
			$button .= vam_draw_hidden_field($k, $v);
		}					

		$process_button_string .= $button;
					
      $process_button_string .= vam_draw_hidden_field('P_SIGN', $sign) .
                               vam_draw_hidden_field('DESC', substr($_SESSION['cart_psbank_id'], strpos($_SESSION['cart_psbank_id'], '-')+1));
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_PSBANK_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_PSBANK_STATUS', 'True', '6', '3', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PSBANK_ALLOWED', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PSBANK_TERMINAL', '', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PSBANK_MERCHANT', '', '6', '6', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PSBANK_SORT_ORDER', '0', '6', '7', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_PSBANK_ZONE', '0', '6', '8', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PSBANK_SECRET_KEY', '', '6', '9', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_PSBANK_ORDER_STATUS_ID', '0', '6', '10', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_PSBANK_TEST', 'test', '6', '11', 'vam_cfg_select_option(array(\'test\', \'production\'), ', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_PSBANK_STATUS', 'MODULE_PAYMENT_PSBANK_ALLOWED', 'MODULE_PAYMENT_PSBANK_TERMINAL', 'MODULE_PAYMENT_PSBANK_MERCHANT', 'MODULE_PAYMENT_PSBANK_SORT_ORDER', 'MODULE_PAYMENT_PSBANK_ZONE', 'MODULE_PAYMENT_PSBANK_SECRET_KEY', 'MODULE_PAYMENT_PSBANK_ORDER_STATUS_ID', 'MODULE_PAYMENT_PSBANK_TEST');
    }
