<?php
/*------------------------------------------------------------------------------
  $Id: sberbank.php 1406 2017-12-15 00:00:08 dependab1e $
   ---------------------------------------------------------------------------------------*/
...
// class constructor
    function __construct() {
      ...
      $this->code = 'sberbank';
      $this->title = MODULE_PAYMENT_SBERBANK_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_SBERBANK_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_SBERBANK_TEXT_ADMIN_DESCRIPTION;
      $this->icon = DIR_WS_ICONS . 'sberbank.png';
      $this->sort_order = MODULE_PAYMENT_SBERBANK_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_SBERBANK_STATUS == 'True') ? true : false);
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_SBERBANK_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_SBERBANK_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
		///Was ist das ?
      if (isset($_SESSION['cart_sberbank_id'])) {
        $order_id = substr($_SESSION['cart_sberbank_id'], strpos($_SESSION['cart_sberbank_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_sberbank_id']);
        }
      }
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_sberbank_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($_SESSION['cart_sberbank_id'], 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_sberbank_id'] = $cartID . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_SBERBANK_TEXT_DESCRIPTION);
    }
    ...
    function process_button() {
		...
		$arr = array('orderNumber' => substr($_SESSION['cart_sberbank_id'], strpos($_SESSION['cart_sberbank_id'], '-')+1),
			'userName' => MODULE_PAYMENT_SBERBANK_UNAME, 'password' => MODULE_PAYMENT_SBERBANK_PASS, 'amount' => $order_sum,
			'failUrl' =>  vam_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL'),
			'returnUrl' => vam_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL'),
		);
		// передадим корзину для онлайн кассы
		$bsk = array();
		for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
			$bsk[] = array('positionId' => $i+1, 'name' => $order->products[$i]['name'], 
				'quantity'  => array('value'=>$order->products[$i]['qty'], 'measure'=>'шт'),
				/*'itemAmount'=> $order->products[$i]['qty'] * $order->products[$i]['final_price'],*/
				'itemCode'  => $order->products[$i]['model'], 'itemPrice'=> round($order->products[$i]['price'] * 100),
			);
		};
		if($order->info['total'] > $order->info['subtotal']) {
			# добавим доставочку
			$bsk[] = array('positionId' => $i+1, 'name' => 'Доставка', 
		...
		$this->form_action_url = $response->formUrl;
		$process_button_string = '<script>document.location.href="'.$response->formUrl.'";document.checkoutUrl.method="get"</script>';
...
// unregister session variables used during checkout
    ...
    function check() {
      ...
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_SBERBANK_STATUS'");
