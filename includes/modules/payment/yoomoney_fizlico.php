<?php
/* -----------------------------------------------------------------------------------------
   $Id: yoomoney_fizlico.php 998 2007/02/07 13:24:46 VaM $
...
// class constructor
    function __construct() {
      ...
      $this->enabled = ((MODULE_PAYMENT_YOOMONEY_FIZLICO_STATUS == 'True') ? true : false);

        $this->form_action_url = 'https://yoomoney.ru/quickpay/confirm.xml';
    }
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_YOOMONEY_FIZLICO_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_YOOMONEY_FIZLICO_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_yoomoney_fizlico_id'])) {
        $order_id = substr($_SESSION['cart_yoomoney_fizlico_id'], strpos($_SESSION['cart_yoomoney_fizlico_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_yoomoney_fizlico_id']);
        }
      }
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_yoomoney_fizlico_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($_SESSION['cartID'] != substr($_SESSION['cart_yoomoney_fizlico_id'], 0, strlen($_SESSION['cartID']))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_yoomoney_fizlico_id'] = $_SESSION['cartID'] . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_YOOMONEY_FIZLICO_TEXT_DESCRIPTION);
    }
    ...
    function process_button() {
      ...
      $process_button_string = vam_draw_hidden_field('receiver', MODULE_PAYMENT_YOOMONEY_FIZLICO_ID) .
                               vam_draw_hidden_field('quickpay-form', 'shop') .
                               vam_draw_hidden_field('targets', STORE_NAME . ' - ' . TEXT_DUPLICATE_ORDER_ORDER_NUMBER . ' ' . substr($_SESSION['cart_yoomoney_fizlico_id'], strpos($_SESSION['cart_yoomoney_fizlico_id'], '-')+1)) .
                               vam_draw_hidden_field('paymentType', 'AC') .
                               vam_draw_hidden_field('sum', number_format($order->info['total'],0,'.','')) .
                               //vam_draw_hidden_field('formcomment', STORE_NAME . ' - ' . TEXT_DUPLICATE_ORDER_ORDER_NUMBER . ' ' . substr($_SESSION['cart_yoomoney_fizlico_id'], strpos($_SESSION['cart_yoomoney_fizlico_id'], '-')+1)) .
                               //vam_draw_hidden_field('short-desc', STORE_NAME . ' - ' . TEXT_DUPLICATE_ORDER_ORDER_NUMBER . ' ' . substr($_SESSION['cart_yoomoney_fizlico_id'], strpos($_SESSION['cart_yoomoney_fizlico_id'], '-')+1)) .
                               vam_draw_hidden_field('label', substr($_SESSION['cart_yoomoney_fizlico_id'], strpos($_SESSION['cart_yoomoney_fizlico_id'], '-')+1)) .
                               //vam_draw_hidden_field('comment', STORE_NAME . ' - ' . TEXT_DUPLICATE_ORDER_ORDER_NUMBER . ' ' . substr($_SESSION['cart_yoomoney_fizlico_id'], strpos($_SESSION['cart_yoomoney_fizlico_id'], '-')+1)) .
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_YOOMONEY_FIZLICO_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_YOOMONEY_FIZLICO_STATUS', 'True', '6', '1', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_YOOMONEY_FIZLICO_ALLOWED', '', '6', '2', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_YOOMONEY_FIZLICO_ID', '', '6', '3', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_YOOMONEY_FIZLICO_SORT_ORDER', '0', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_YOOMONEY_FIZLICO_ZONE', '0', '6', '5', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_YOOMONEY_FIZLICO_SECRET_KEY', '', '6', '6', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_YOOMONEY_FIZLICO_ORDER_STATUS_ID', '0', '6', '7', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_YOOMONEY_FIZLICO_STATUS', 'MODULE_PAYMENT_YOOMONEY_FIZLICO_ALLOWED', 'MODULE_PAYMENT_YOOMONEY_FIZLICO_ID', 'MODULE_PAYMENT_YOOMONEY_FIZLICO_SORT_ORDER', 'MODULE_PAYMENT_YOOMONEY_FIZLICO_ZONE', 'MODULE_PAYMENT_YOOMONEY_FIZLICO_SECRET_KEY', 'MODULE_PAYMENT_YOOMONEY_FIZLICO_ORDER_STATUS_ID');
    }
