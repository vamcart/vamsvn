<?php
/* -----------------------------------------------------------------------------------------
   $Id: paykeeper.php 998 2007/02/07 13:24:46 VaM $
...
// class constructor
    function __construct() {
      ...
      $this->code = 'paykeeper';
      $this->title = MODULE_PAYMENT_PAYKEEPER_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_PAYKEEPER_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_PAYKEEPER_TEXT_ADMIN_DESCRIPTION;
      $this->icon = DIR_WS_ICONS . 'paykeeper.png';
      $this->sort_order = MODULE_PAYMENT_PAYKEEPER_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_PAYKEEPER_STATUS == 'True') ? true : false);

        $this->form_action_url = MODULE_PAYMENT_PAYKEEPER_ID;
    }
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_PAYKEEPER_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_PAYKEEPER_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_paykeeper_id'])) {
        $order_id = substr($_SESSION['cart_paykeeper_id'], strpos($_SESSION['cart_paykeeper_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_paykeeper_id']);
        }
      }
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_paykeeper_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($_SESSION['cartID'] != substr($_SESSION['cart_paykeeper_id'], 0, strlen($_SESSION['cartID']))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_paykeeper_id'] = $_SESSION['cartID'] . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_PAYKEEPER_TEXT_DESCRIPTION);
    }
    ...
    function process_button() {
      ...
      $process_button_string = '';
                               ...
                               $purse = MODULE_PAYMENT_PAYKEEPER_WMR;
                               $order_sum = $order->info['total'];

        $sum = $order->info['total'];
        $orderid = substr($_SESSION['cart_paykeeper_id'], strpos($_SESSION['cart_paykeeper_id'], '-')+1);
        $clientid = $order->customer['firstname'] . ' ' . $order->customer['lastname'];
        $client_email = $order->customer['email_address'];
        $client_phone = $order->customer['telephone'];
        $service_name = STORE_NAME;

        $secret = MODULE_PAYMENT_PAYKEEPER_SECRET_KEY;
      ...
      $process_button_string = vam_draw_hidden_field('orderid', substr($_SESSION['cart_paykeeper_id'], strpos($_SESSION['cart_paykeeper_id'], '-')+1)) .
                               vam_draw_hidden_field('client_email', $client_email) .
                               vam_draw_hidden_field('client_phone', $client_phone) .
                               vam_draw_hidden_field('service_name', $service_name) .
                               vam_draw_hidden_field('clientid', $clientid) .
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_PAYKEEPER_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_PAYKEEPER_STATUS', 'True', '6', '3', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PAYKEEPER_ALLOWED', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PAYKEEPER_ID', '', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PAYKEEPER_SORT_ORDER', '0', '6', '7', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_PAYKEEPER_ZONE', '0', '6', '8', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PAYKEEPER_SECRET_KEY', '', '6', '9', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_PAYKEEPER_ORDER_STATUS_ID', '0', '6', '10', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_PAYKEEPER_STATUS', 'MODULE_PAYMENT_PAYKEEPER_ALLOWED', 'MODULE_PAYMENT_PAYKEEPER_ID', 'MODULE_PAYMENT_PAYKEEPER_SORT_ORDER', 'MODULE_PAYMENT_PAYKEEPER_ZONE', 'MODULE_PAYMENT_PAYKEEPER_SECRET_KEY', 'MODULE_PAYMENT_PAYKEEPER_ORDER_STATUS_ID');
    }
