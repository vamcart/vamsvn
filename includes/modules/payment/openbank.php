<?php
/* -----------------------------------------------------------------------------------------
   $Id: openbank.php 2588 2010/04/13 13:24:46 oleg_vamsoft $
...
// class constructor
    public function __construct() {
      ...
      $this->code = 'openbank';
      $this->title = MODULE_PAYMENT_OPENBANK_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_OPENBANK_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_OPENBANK_TEXT_ADMIN_DESCRIPTION;
      $this->icon = DIR_WS_ICONS . 'openbank.png';
      $this->sort_order = MODULE_PAYMENT_OPENBANK_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_OPENBANK_STATUS == 'True') ? true : false);
...
// class methods
    public function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_OPENBANK_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_OPENBANK_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_openbank_id'])) {
        $order_id = substr($_SESSION['cart_openbank_id'], strpos($_SESSION['cart_openbank_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_openbank_id']);
        }
      }
    ...
    public function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_openbank_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($_SESSION['cart_openbank_id'], 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_openbank_id'] = $cartID . '-' . $insert_id;
        }

      }

// Переход на платёжную форму банка открытие
		vam_redirect('https://secure.openbank.ru/openapi/deploy/open_payments/index.html?partnerId='.MODULE_PAYMENT_OPENBANK_SHOP_ID.'&serviceId='.MODULE_PAYMENT_OPENBANK_SERVICE_ID.'&account='.substr($_SESSION['cart_openbank_id'], strpos($_SESSION['cart_openbank_id'], '-')+1).'&amount='.$order->info['total'].'&emailEnabled=1&email='.$order->customer['email_address'].'&phoneEnabled=1&phone='.$order->customer['telephone'].'&supportEmail='.STORE_OWNER_EMAIL_ADDRESS.'&partnerBackUrl='.vam_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL').'&amountReadOnly=1&paramsReadOnly=1');

      return array('title' => MODULE_PAYMENT_OPENBANK_TEXT_DESCRIPTION);
    }
...
// unregister session variables used during checkout
    ...
    public function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_OPENBANK_STATUS'");
    ...
    public function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_OPENBANK_STATUS', 'True', '6', '1', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_OPENBANK_ALLOWED', '', '6', '2', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_OPENBANK_SHOP_ID', '', '6', '3', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_OPENBANK_SERVICE_ID', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_OPENBANK_SORT_ORDER', '0', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_OPENBANK_ZONE', '0', '6', '6', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_OPENBANK_ORDER_STATUS_ID', '0', '6', '7', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
    ...
    public function keys() {
      return array('MODULE_PAYMENT_OPENBANK_STATUS', 'MODULE_PAYMENT_OPENBANK_ALLOWED', 'MODULE_PAYMENT_OPENBANK_SHOP_ID', 'MODULE_PAYMENT_OPENBANK_SERVICE_ID', 'MODULE_PAYMENT_OPENBANK_SORT_ORDER', 'MODULE_PAYMENT_OPENBANK_ZONE', 'MODULE_PAYMENT_OPENBANK_ORDER_STATUS_ID');
    }
