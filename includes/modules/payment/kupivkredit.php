      ...
      $process_button_string = '';
		...
		$items = array();
			...
			foreach ($order->products as $product) {
				...
				$items[] = array(
					'title' => $product['name'],
					'category' => $this->get_category_name($this->get_category_id($product['id'])),
					'qty' => $product['qty'],
		...
		$order = array (
		  ...
		  'partnerOrderId' => substr($_SESSION['cart_kupivkredit_id'], strpos($_SESSION['cart_kupivkredit_id'], '-')+1),
		  'deliveryType' => ''
		);

    $base64 = base64_encode(json_encode($order));
    $sig = $this->signMessage($base64, MODULE_PAYMENT_KUPIVKREDIT_SECRET_KEY);
