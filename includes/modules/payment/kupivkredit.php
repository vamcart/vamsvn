<?php
/* -----------------------------------------------------------------------------------------
   $Id: kupivkredit.php 998 2007/02/07 13:24:46 VaM $
...
// class methods
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_kupivkredit_id'])) {
        $order_id = substr($_SESSION['cart_kupivkredit_id'], strpos($_SESSION['cart_kupivkredit_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_kupivkredit_id']);
        }
      }
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_kupivkredit_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($_SESSION['cartID'] != substr($_SESSION['cart_kupivkredit_id'], 0, strlen($_SESSION['cartID']))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_kupivkredit_id'] = $_SESSION['cartID'] . '-' . $insert_id;
        }
      }
...
$i = 0;
            ...
            foreach ($order->products as $product) {
                               ...
                               $process_button_string .= vam_draw_hidden_field('itemPrice_'.$i, number_format($product['price'], 2, '.', ''));
...
// unregister session variables used during checkout
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_KUPIVKREDIT_STATUS', 'True', '6', '3', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_KUPIVKREDIT_ALLOWED', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_KUPIVKREDIT_SHOPID', 'test_online', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_KUPIVKREDIT_SHOWCASEID', 'test_online', '6', '6', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_KUPIVKREDIT_SORT_ORDER', '0', '6', '7', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_KUPIVKREDIT_ZONE', '0', '6', '8', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_KUPIVKREDIT_PROMOCODE', 'default', '6', '9', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_KUPIVKREDIT_ORDER_STATUS_ID', '0', '6', '10', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_KUPIVKREDIT_STATUS', 'MODULE_PAYMENT_KUPIVKREDIT_ALLOWED', 'MODULE_PAYMENT_KUPIVKREDIT_SHOPID', 'MODULE_PAYMENT_KUPIVKREDIT_SHOWCASEID', 'MODULE_PAYMENT_KUPIVKREDIT_SORT_ORDER', 'MODULE_PAYMENT_KUPIVKREDIT_ZONE', 'MODULE_PAYMENT_KUPIVKREDIT_PROMOCODE', 'MODULE_PAYMENT_KUPIVKREDIT_ORDER_STATUS_ID');
    }
