...
$sql_tags = "
    SELECT
        f.*
    FROM " . TABLE_TAGS . " f , " . TABLE_TAGS_TO_PRODUCTS . " f2c
    WHERE
         f2c.products_id = '" . (int)$tags_category_id . "'
...
$tagsI=0; $tagsIcon='';
//echo strpos($one_tag['description'],'src="')." ";
...
if (sizeof($module_content_tags) > 0) {
	...
	// set cache ID
	 if (!CacheCheck()) {
		$module->caching = 0;
      $module= $module->fetch(CURRENT_TEMPLATE.'/module/tags_product_info.html');
	} else {
        $module->caching = 1;
        $module->cache_lifetime=CACHE_LIFETIME;
        $module->cache_modified_check=CACHE_CHECK;
        $module = $module->fetch(CURRENT_TEMPLATE.'/module/tags_product_info.html',$cache_id);
	}
	$info->assign('MODULE_tags', $module);
}
?>