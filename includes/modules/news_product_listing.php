...
if (sizeof($module_listing_content_news) > 0) {
	...
	// set cache ID
	 if (!CacheCheck()) {
      ...
      $module_listing= $module_listing->fetch(CURRENT_TEMPLATE.'/module/latest_news_product_listing.html');
	} else {
        $module_listing->caching = 1;
        $module_listing->cache_lifetime=CACHE_LIFETIME;
        $module_listing->cache_modified_check=CACHE_CHECK;
        $module_listing = $module_listing->fetch(CURRENT_TEMPLATE.'/module/latest_news_product_listing.html',$cache_id);
	}
