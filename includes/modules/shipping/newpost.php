  ...
  class newpost {
    ...
    function quote($method = '') {
      global $order, $shipping_weight;
	    ...
	    $ret = $this->newpost_api_calc($aut_login, $auth_Password, $date_Execute, $sender_postcode, $order->delivery['postcode'], $total_weight, $receiverCityId);
		//print_r	($ret);	
	    $shipping_cost = 0;
      ...
      // если вес больше указаного в переменой то: 
		...
		$shipping_cost = $shipping_cost + MODULE_SHIPPING_NEWPOST_COST;
		} else {
        $shipping_cost = $shipping_cost + MODULE_SHIPPING_NEWPOST_COST_2; 
        }
		...
		$ret_pvz = $this->newpost_api_pvz($receiverCityId);
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_NEWPOST_COST', '150', '6', '0', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_NEWPOST_COST_2', '100', '6', '0', now())");
	   vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_NEWPOST_API_LOGIN', '', '6', '0', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_NEWPOST_ZIP', '', '6', '0', now())");
    ...
    function keys() {
      return array('MODULE_SHIPPING_NEWPOST_STATUS', 'MODULE_SHIPPING_NEWPOST_COST', 'MODULE_SHIPPING_NEWPOST_COST_2', 'MODULE_SHIPPING_NEWPOST_API_LOGIN', 'MODULE_SHIPPING_NEWPOST_ZIP','MODULE_SHIPPING_NEWPOST_ALLOWED', 'MODULE_SHIPPING_NEWPOST_TAX_CLASS', 'MODULE_SHIPPING_NEWPOST_ZONE', 'MODULE_SHIPPING_NEWPOST_SORT_ORDER', 'MODULE_SHIPPING_SDEK_MIN_SUM', 'MODULE_SHIPPING_SDEK_PROCENT', 'MODULE_SHIPPING_SDEK_MIN_SUM_ORDER');
    }
