...
for ($i = 0, $n = sizeof($products); $i < $n; $i ++) {
	...
	'PRODUCTS_QTY' => vam_draw_input_field('cart_quantity[]', $products[$i]['quantity'], 'size="2" id="cart" class="text-center item-quantity input-small"').vam_draw_hidden_field('products_id[]', $products[$i]['id'],'class="ajax_qty"').vam_draw_hidden_field('old_qty[]', $products[$i]['quantity']), 
	...
	'BOX_DELETE' => '<button class="button btn btn-dark add-to-cart cart_delete" type="button" value="'.$products[$i]['id'].'"><span>X</span></button>', 
   'PLUS' => '<button class="button btn btn-dark cart_change cart_plus" type="button" value="1"><span>+</span></button>',
   'MINUS' => '<button class="button btn btn-dark cart_change cart_minus" type="button" value="-1"><span>-</span></button>',
