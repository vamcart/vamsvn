...
for ($i = 0, $n = sizeof($productss[$key]['products']); $i < $n; $i ++) {
	...
	'BOX_DELETE' => '<button class="button btn btn-secondary add-to-cart cart_delete" type="button" value="'.$productss[$key]['products'][$i]['id'].'"><span>X</span></button>', 
   'PLUS' => '<button class="button btn btn-secondary cart_change cart_plus" type="button" value="1"><span>+</span></button>',
   'MINUS' => '<button class="button btn btn-secondary cart_change cart_minus" type="button" value="-1"><span>-</span></button>',
...
$module->caching = 0;
$module = $module->fetch(CURRENT_TEMPLATE.'/module/order_details_checkout.html');

$vamTemplate->assign('MODULE_order_details_checkout', $module);
?>