...
$author_query = "select authors_id, authors_name from ".TABLE_AUTHORS." where authors_id = '".(int)$_GET['authors_id']."'";
$author_query = vam_db_query($author_query);
$author = vam_db_fetch_array($author_query);

if (isset ($_GET['action']) && $_GET['action'] == 'process' && $spam_flag == false) {
	if ($author['authors_id'] > 0 && $author['authors_name'] != '') { // We got to the process but it is an illegal product, don't write
    ...
    if ($error == false) {
		...
		vam_db_query("insert into ".TABLE_AUTHOR_REVIEWS." (authors_id, customers_id, customers_name, reviews_rating, date_added) values ('".(int)$_GET['authors_id']."', '".(int) $_SESSION['customer_id']."', '".addslashes($customer_values['customers_firstname']).' '.addslashes($customer_values['customers_lastname'])."', '".addslashes($_POST['rating'])."', now())");
		$insert_id = vam_db_insert_id();
		vam_db_query("insert into ".TABLE_AUTHOR_REVIEWS_DESCRIPTION." (reviews_id, languages_id, reviews_text) values ('".$insert_id."', '".(int) $_SESSION['languages_id']."', '".addslashes($_POST['review'])."')");


	vam_redirect(vam_href_link(FILENAME_ARTICLES, $_POST['get_params']));
	}
 }
}
...
if (!vam_db_num_rows($author_query)) {
	...
	$vamTemplate->assign('FORM_ACTION', vam_draw_form('author_reviews_write', vam_href_link(FILENAME_AUTHOR_REVIEWS_WRITE, 'action=process&authors_id='.$_GET['authors_id']), 'post', 'onsubmit="return checkForm();"'));
...
$main_content = $vamTemplate->fetch(CURRENT_TEMPLATE.'/module/author_reviews_write.html');

$vamTemplate->assign('language', $_SESSION['language']);
$vamTemplate->assign('main_content', $main_content);
$vamTemplate->caching = 0;
if (!defined(RM)) $vamTemplate->loadFilter('output', 'note');
$template = (file_exists('templates/'.CURRENT_TEMPLATE.'/'.FILENAME_AUTHOR_REVIEWS_WRITE.'.html') ? CURRENT_TEMPLATE.'/'.FILENAME_PRODUCT_AUTHOR_WRITE.'.html' : CURRENT_TEMPLATE.'/index.html');
