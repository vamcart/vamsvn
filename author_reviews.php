...
$product_info_query = vam_db_query("select pd.authors_name from ".TABLE_AUTHORS." pd where pd.authors_id = '".(int) $_GET['authors_id']."'");
...
$breadcrumb->add(NAVBAR_TITLE_PRODUCT_REVIEWS, vam_href_link(FILENAME_AUTHOR_REVIEWS, $get_params));

require (DIR_WS_INCLUDES.'header.php');

$vamTemplate->assign('PRODUCTS_NAME', $product_info['authors_name']);

$data_reviews = array ();
$reviews_query = vam_db_query("select reviews_rating, reviews_id, customers_name, date_added, last_modified, reviews_read from ".TABLE_AUTHOR_REVIEWS." where authors_id = '".(int) $_GET['authors_id']."' order by reviews_id DESC");
...
$vamTemplate->assign('BUTTON_BACK', '<a class="button" href="'.vam_href_link(FILENAME_ARTICLES, $get_params_back).'">'.vam_image_button('back.png', IMAGE_BUTTON_BACK).'</a>');
$vamTemplate->assign('BUTTON_WRITE', '<a class="button" href="'.vam_href_link(FILENAME_AUTHORS_REVIEWS_WRITE, $get_params).'">'.vam_image_button('add.png', IMAGE_BUTTON_WRITE_REVIEW).'</a>');
...
// set cache ID
 if (!CacheCheck()) {
	...
	$main_content = $vamTemplate->fetch(CURRENT_TEMPLATE.'/module/author_reviews.html');
} else {
	$vamTemplate->caching = 1;
	$vamTemplate->cache_lifetime = CACHE_LIFETIME;
	$vamTemplate->cache_modified_check = CACHE_CHECK;
	$cache_id = $_SESSION['language'].$_GET['authors_id'];
	$main_content = $vamTemplate->fetch(CURRENT_TEMPLATE.'/module/author_reviews.html', $cache_id);
}
...
$template = (file_exists('templates/'.CURRENT_TEMPLATE.'/'.FILENAME_AUTHOR_REVIEWS.'.html') ? CURRENT_TEMPLATE.'/'.FILENAME_AUTHOR_REVIEWS.'.html' : CURRENT_TEMPLATE.'/index.html');
