...
function vam_categories_left_get_category_products( $cat_id )
{
...
function vam_categories_left_get_subcategory( $owner_cat_id, $owner_cat_name = '', $owner_cat_image = '', $current_cPath = '', $level = 0 )
{
    ...
    if ( $categories_current_level )
    {
        ...
        {
        	$level++;
            vam_categories_left_get_subcategory(
                $v[ 'id' ],
                $v[ 'name' ],
                $v[ 'image' ],
                $v[ 'path' ],
...
//        vam_categories_left_get_category_products( $owner_cat_id );
...
}  // function vam_categories_left_get_subcategory( ... )
...
if( !$box->isCached( CURRENT_TEMPLATE . '/boxes/box_categories_left.html', $cache_id ) || !$cache )
{
...
//  require_once ( DIR_FS_INC . 'vam_has_category_subcategories.inc.php' );
  ...
  vam_categories_left_get_subcategory( 0 );
...
if ( !$cache )
{
	$box_categories_left = $box->fetch( CURRENT_TEMPLATE . '/boxes/box_categories_left.html' );
} else {
	$box_categories_left = $box->fetch( CURRENT_TEMPLATE . '/boxes/box_categories_left.html', $cache_id );
};

$module->assign( 'box_CATEGORIES_LEFT', $box_categories_left );
?>