...
if ($_SESSION['wishlist']->count_contents() > 0) {
	...
	for ($i = 0, $n = sizeof($products); $i < $n; $i ++) {
		...
		// Push all attributes information in an array
	        ...
	        $product_attributes_in_wishlist = "";
		...
		if (isset ($products[$i]['attributes'])) {
			foreach ($products[$i]['attributes'] as $option => $value) {
				...
				$product_attributes_in_wishlist .= vam_draw_hidden_field('id[' . $products[$i]['id'] . '][' . $option . ']', $attributes_values['options_values_id']);
			}
		}
		// Push all attributes information in an array
		$products_in_wishlist[] = array ('QTY' => $products[$i]['quantity'], 
									 'ID' => $products[$i]['id'], 
...
'PRODUCTS_QTY' => $product_attributes_in_wishlist . vam_draw_hidden_field('wishlist_quantity[]', $products[$i]['quantity'] - 1) . vam_draw_hidden_field('products_id[]', $products[$i]['id']) . vam_draw_hidden_field('old_qty[]', $products[$i]['quantity']) . ($products[$i]['quantity'] < 2 ? vam_draw_hidden_field('wishlist_delete[]', $products[$i]['id']) : ''),
...
if ($_SESSION['customers_status']['customers_status_show_price'] == '1') {
	...
	$box->assign('TOTAL_QUANTITY', $_SESSION['wishlist']->count_contents());
} 
	

	$box->assign('UST', $_SESSION['wishlist']->show_tax());
...
// GV Code End
$box->assign('LINK_HOME', vam_href_link(FILENAME_DEFAULT, '', 'SSL'));
$box->assign('LINK_WISHLIST', vam_href_link(FILENAME_WISHLIST, '', 'SSL'));
$box->assign('LINK_CHECKOUT', vam_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL'));
$box->assign('products', $products_in_wishlist);

$box->caching = 0;
$box->assign('language', $_SESSION['language']);
$box_wishlist = $box->fetch(CURRENT_TEMPLATE.'/boxes/box_wishlist.html');
$vamTemplate->assign('box_WISHLIST', $box_wishlist);
?>