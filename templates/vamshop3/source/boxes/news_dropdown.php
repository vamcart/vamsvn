...
if (sizeof($module_content) > 0) {
    ...
    if (USE_CACHE=='false') {
        $box->caching = 0;
        $module= $box->fetch(CURRENT_TEMPLATE.'/boxes/box_latest_news_dropdown.html');
    } else {
        $box->caching = 1;
        $box->cache_lifetime=CACHE_LIFETIME;
        $box->cache_modified_check=CACHE_CHECK;
        $module = $box->fetch(CURRENT_TEMPLATE.'/boxes/box_latest_news_dropdown.html',$cache_id);
    }
    $vamTemplate->assign('box_LATESTNEWS_DROPDOWN',$module);
}
?>