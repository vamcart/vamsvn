...
if ($process == false) {
	...
	if ($addresses_count > 1) {
		...
		while ($addresses = vam_db_fetch_array($addresses_query)) {
			...
			$address_content .= '<p><span class="bold">'.$addresses['firstname'].' '.$addresses['lastname'].'</span>&nbsp;'.vam_draw_radio_field('address', $addresses['address_book_id'], ($addresses['address_book_id'] == $_SESSION['sendto'])).'</p>
