...
if(isset($_GET[AM_ACTION_GET_VARIABLE]) && $_GET[AM_ACTION_GET_VARIABLE] == 'prompt') {
	...
	switch($_GET['section']) {
		...
		case 'amAddNewOptionValueToProduct':
			...
			$amPopup->addToContents(languageTextFields());
			$amPopup->addToContents(updateCancelButtons($_GET['section']));
			$amPopup->addToContents(vam_draw_hidden_field('option_id',$arrExtraValues['option_id'],'id="option_id"'));
			echo $amPopup->output();
			break;
		...
		case 'amRemoveOptionValueFromProduct':
			...
			$amPopup->addToContents(yesNoButtons($_GET['section']));
			$amPopup->addToContents(vam_draw_hidden_field('option_id',$arrExtraValues['option_id'],'id="option_id"'));
			$amPopup->addToContents(vam_draw_hidden_field('option_value_id',$arrExtraValues['option_value_id'],'id="option_value_id"'));
			echo $amPopup->output();
			break;
//----------------------------
...
//-----------------------------
		case 'amAddNewDownloadForProduct':
			...
			$amPopup->addToContents('<div class="amFieldLabel">' .AM_AJAX_FILENAME.'</div><div class="amField">'. vam_draw_input_field('products_attributes_filename','','id="products_attributes_filename"') );
...
/*			$amPopup->addToContents('<a href="#" onClick="mcFileManager.open(document.forms[0].name,\'products_attributes_filename\',\'\',\'\',{document_base_url : \''.  DIR_WS_CATALOG . 'download/\',relative_urls:true,remove_script_host:true,rootpath:\''.DIR_FS_CATALOG.'download/\',remember_last_path:false});">' );
			$amPopup->addToContents( vam_image(DIR_WS_ADMIN. DIR_WS_INCLUDES . 'tiny_mce/themes/advanced/images/browse.gif', 'browse' ));
			$amPopup->addToContents('</a>');
*/			// EOF only for TinyMCE filemanager
			$amPopup->addToContents('</div>');
			$amPopup->addToContents('</DIV>');
			$amPopup->addToContents('<DIV class="amFieldset">');
			$amPopup->addToContents('<div class="amFieldLabel">' .AM_AJAX_FILE_DAYS.'</div><div class="amField">'. vam_draw_input_field('products_attributes_maxdays','7','id="products_attributes_maxdays"') . '</div>');
			$amPopup->addToContents('</DIV>');
			$amPopup->addToContents('<DIV class="amFieldset">');
			$amPopup->addToContents('<div class="amFieldLabel">' .AM_AJAX_FILE_COUNT.'</div><div class="amField">'. vam_draw_input_field('products_attributes_maxcount','5','id="products_attributes_maxcount"') . '</div>');
			//$amPopup->addToContents($_GET['section']);
			$amPopup->addToContents('</DIV>');
			$amPopup->addToContents(updateCancelButtons($_GET['section']));
			$amPopup->addToContents(vam_draw_hidden_field('option_id',$arrExtraValues['option_id'],'id="option_id"'));
			$amPopup->addToContents(vam_draw_hidden_field('products_attributes_id',$arrExtraValues['products_attributes_id'],'id="products_attributes_id"'));
			echo $amPopup->output();
			break;
		case 'amEditDownloadForProduct':
			...
			//$amPopup->addToContents(languageTextFields());
			$amPopup->addToContents('<DIV class="amFieldset">');
			$amPopup->addToContents('<div class="amFieldLabel">' .AM_AJAX_FILENAME.'</div><div class="amField">'.vam_draw_input_field('products_attributes_filename',$arrExtraValues['products_attributes_filename'],'id="products_attributes_filename"') );
			// only for TinyMCE filemanager
/*			$amPopup->addToContents('<a href="#" onClick="mcFileManager.open(document.forms[0].name,\'products_attributes_filename\',\'\',\'\',{document_base_url : \''.  DIR_WS_CATALOG . 'download/\',relative_urls:true,remove_script_host:true,rootpath:\''.DIR_FS_CATALOG.'download/\',remember_last_path:false});">' );
			$amPopup->addToContents( vam_image(DIR_WS_ADMIN. DIR_WS_INCLUDES . 'tiny_mce/themes/advanced/images/browse.gif', 'browse' ));
			$amPopup->addToContents('</a>');
*/			// EOF only for TinyMCE filemanager
			...
			$amPopup->addToContents('</DIV>');
			$amPopup->addToContents('<DIV class="amFieldset">');
			$amPopup->addToContents('<div class="amFieldLabel">' .AM_AJAX_FILE_DAYS.'</div><div class="amField">' .vam_draw_input_field('products_attributes_maxdays',$arrExtraValues['products_attributes_maxdays'],'id="products_attributes_maxdays"') . '</div>');
			$amPopup->addToContents('</DIV>');
			$amPopup->addToContents('<DIV class="amFieldset">');
			$amPopup->addToContents('<div class="amFieldLabel">' .AM_AJAX_FILE_COUNT.'</div><div class="amField">' .vam_draw_input_field('products_attributes_maxcount',$arrExtraValues['products_attributes_maxcount'],'id="products_attributes_maxcount"') . '</div>');
			$amPopup->addToContents('</DIV>');
			//$amPopup->addToContents($_GET['section']);
			$amPopup->addToContents('</DIV>');
			$amPopup->addToContents(updateCancelButtons($_GET['section']));
...
// For QT Pro Plugin -- added by Phocea
		...
		case 'saveTemplate':
			...
			$amPopup->addToContents(AM_AJAX_NEW_NAME.'&nbsp'.vam_draw_input_field('template_name','','id="template_name" onchange="((this.value != \'\') ? document.getElementById(\'existing_template\').selectedIndex = 0 : \'\')"'));
