...
// Get last order id for checkout_success
    ...
    $orders_query = vamDBquery($orders_query_raw);
    $orders_values = vam_db_fetch_array($orders_query,true);
...
// Now get all downloadable products in that order
  ...
  $downloads_query = vamDBquery($downloads_query_raw);

// Don't display if there is no downloadable product
  if (vam_db_num_rows($downloads_query,true) > 0) {

    while ($downloads_values = vam_db_fetch_array($downloads_query,true)) {
