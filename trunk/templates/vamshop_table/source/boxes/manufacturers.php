...
/* -----------------------------------------------------------------------------------------
   ...
   (c) 2004	 xt:Commerce (manufacturers.php,v 1.9 2003/08/17); xt-commerce.com 
...
if (!$box->is_cached(CURRENT_TEMPLATE.'/boxes/box_manufacturers.html', $cache_id) || !$cache) {
	...
	if (vam_db_num_rows($manufacturers_query, true) <= MAX_DISPLAY_MANUFACTURERS_IN_A_LIST) {
		...
		$box_content = vam_draw_form('manufacturers', vam_href_link(FILENAME_DEFAULT, '', 'NONSSL', false), 'get').vam_draw_pull_down_menu('manufacturers_id', $manufacturers_array, $_GET['manufacturers_id'], 'onchange="this.form.submit();" size="'.MAX_MANUFACTURERS_LIST.'" style="width: 100%"').vam_hide_session_id().'</form>';
