...
// class methods
    ...
    function confirmation() {
      global $cartID, $cart_rbkmoney_id, $customer_id, $languages_id, $order, $order_total_modules;
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_rbkmoney_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($cart_rbkmoney_id, 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
if ($_SERVER["HTTP_X_FORWARDED_FOR"]) {
	...
	$customers_ip = $_SERVER["REMOTE_ADDR"];
          ...
          $cart_rbkmoney_id = $cartID . '-' . $insert_id;
          $_SESSION['cart_rbkmoney_id'] = $cart_rbkmoney_id;
        }
      }
    ...
    function process_button() {
      global $customer_id, $order, $sendto, $vamPrice, $currencies, $cart_rbkmoney_id, $shipping;
      ...
      $process_button_string = vam_draw_hidden_field('eshopId', MODULE_PAYMENT_RBKMONEY_SHOP_ID) .
                               vam_draw_hidden_field('orderId', substr($cart_rbkmoney_id, strpos($cart_rbkmoney_id, '-')+1)) .
                               vam_draw_hidden_field('serviceName', substr($cart_rbkmoney_id, strpos($cart_rbkmoney_id, '-')+1)) .
    ...
    function before_process() {
      global $customer_id, $order, $vamPrice, $order_totals, $sendto, $billto, $languages_id, $payment, $currencies, $cart, $cart_rbkmoney_id;
      global $$payment;
