...
// class constructor
    function pay2pay() {
      ...
      $this->code = 'pay2pay';
      $this->title = MODULE_PAYMENT_PAY2PAY_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_PAY2PAY_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_PAY2PAY_TEXT_ADMIN_DESCRIPTION;
      $this->icon = '';
      $this->sort_order = MODULE_PAYMENT_PAY2PAY_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_PAY2PAY_STATUS == 'True') ? true : false);
      $this->form_action_url = 'https://merchant.pay2pay.com/?page=init';
    }
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_PAY2PAY_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_PAY2PAY_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_pay2pay_id'])) {
        $order_id = substr($_SESSION['cart_pay2pay_id'], strpos($_SESSION['cart_pay2pay_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_pay2pay_id']);
        }
      }
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_pay2pay_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($_SESSION['cart_pay2pay_id'], 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
if ($_SERVER["HTTP_X_FORWARDED_FOR"]) {
	...
	$customers_ip = $_SERVER["REMOTE_ADDR"];
          ...
          $_SESSION['cart_pay2pay_id'] = $cartID . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_PAY2PAY_TEXT_DESCRIPTION);
    }
    ...
    function process_button() {
      ...
      $process_button_string = '';
        ...
        $xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>".
              "<request>".
              "  <version>1.2</version>".
              "  <merchant_id>".MODULE_PAYMENT_PAY2PAY_ID."</merchant_id>".
              "  <language>ru</language>".
              "  <order_id>$order_id</order_id>".
              "  <amount>$order_sum</amount>".
              "  <currency>".$curr['currency']."</currency>".
              "  <description>Оплата заказа №$order_id</description>".
              "  <success_url>".vam_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL')."</success_url>".
              "  <fail_url>".vam_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL')."</fail_url>".
              "  <result_url>".vam_href_link('pay2pay.php', '', 'SSL')."</result_url>";
          if (MODULE_PAYMENT_PAY2PAY_TESTMODE == 'Yes')
            $xml .= "<test_mode>1</test_mode>";
          $xml .= "</request>"; 
                
      $key = MODULE_PAYMENT_PAY2PAY_SECRET_KEY; 

      $operation_xml = base64_encode($xml);
      $signature = base64_encode(md5($key.$xml.$key));        
        
      $process_button_string = vam_draw_hidden_field('xml', $operation_xml) . 
                               vam_draw_hidden_field('sign', $signature);
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_PAY2PAY_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_PAY2PAY_STATUS', 'True', '6', '1', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PAY2PAY_ALLOWED', '', '6', '2', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PAY2PAY_ID', '', '6', '3', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PAY2PAY_SORT_ORDER', '0', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_PAY2PAY_ZONE', '0', '6', '5', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PAY2PAY_SECRET_KEY', '', '6', '6', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PAY2PAY_HIDDEN_KEY', '', '6', '6', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_PAY2PAY_ORDER_STATUS_ID', '0', '6', '7', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_PAY2PAY_TESTMODE', 'Yes', '6', '1', 'vam_cfg_select_option(array(\'Yes\', \'No\'), ', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_PAY2PAY_STATUS', 'MODULE_PAYMENT_PAY2PAY_ALLOWED', 'MODULE_PAYMENT_PAY2PAY_ID', 'MODULE_PAYMENT_PAY2PAY_SORT_ORDER', 'MODULE_PAYMENT_PAY2PAY_ZONE', 'MODULE_PAYMENT_PAY2PAY_SECRET_KEY', 'MODULE_PAYMENT_PAY2PAY_HIDDEN_KEY', 'MODULE_PAYMENT_PAY2PAY_ORDER_STATUS_ID', 'MODULE_PAYMENT_PAY2PAY_TESTMODE');
    }
