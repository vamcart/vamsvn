<?php
/* --------------------------------------------------------------
   $Id: articles_xsell.php 1125 2007-03-09 11:13:01Z VaM $   
              ...
              <tr class="dataTableHeadingRow"> 
			   ...
			   $num_of_articles = sizeof($articles_id);
				for ($i=0; $i < $num_of_articles; $i++)
					...
					echo '<td class="dataTableContent"  valign="top">&nbsp;<a href="' . xtc_href_link(FILENAME_ARTICLES_XSELL, 'add_related_article_ID=' . $articles_id[$i], 'NONSSL') . '">Добавить/Удалить товар к статье</a></td>';
									
					if (count($Related_items)>1)
					{
					  echo '<td class="dataTableContent" valign="top">&nbsp;<a href="' . xtc_href_link(FILENAME_ARTICLES_XSELL, 'sort=1&add_related_article_ID=' . $articles_id[$i], 'NONSSL') . '">Сортировать</a>&nbsp;</td>';
					} else {
...
if ($_POST[xsell_id])
 ...
 ?>
                ...
                <tr>
                  <td class="main"><?php echo sprintf(TEXT_LINK_SORT_PRODUCTS, xtc_href_link(FILENAME_ARTICLES_XSELL, '&sort=1&add_related_article_ID=' . $add_related_article_ID, 'NONSSL')); ?></td>
                </tr>
                <tr>
                  <td><?php echo xtc_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                </tr>
                <tr>
                  <td class="main"><?php echo sprintf(TEXT_LINK_MAIN_PAGE, xtc_href_link(FILENAME_ARTICLES_XSELL, '', 'NONSSL')); ?></td>
                </tr>
                <tr>
                  <td><?php echo xtc_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                </tr>
...
//	  if ($_POST[xsell_id])
	//	echo '<a href="' . xtc_href_link(FILENAME_ARTICLES_XSELL, 'sort=1&add_related_article_ID=' . $_POST[add_related_article_ID], 'NONSSL') . '">Click here to sort (top to bottom) the added cross sale</a>' . "\n";
	}
		
        if ($add_related_article_ID && ! $_POST && !$sort)
	{	?>
 	  <table border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
               <form action="<?php xtc_href_link(FILENAME_ARTICLES_XSELL, '', 'NONSSL'); ?>" method="post">
                <tr class="dataTableHeadingRow">
				...
				$query = "select * from " . TABLE_ARTICLES_XSELL . " where articles_id = '".$add_related_article_ID."'";
					...
					<tr>
                      ...
                      <td bgcolor="#CCCCCC">
                        ...
                        <?php echo '<input type="submit" class="button" onClick="this.blur();" value="' . BUTTON_SAVE . '"/>' . '&nbsp;&nbsp;<a class="button" href="' . xtc_href_link(FILENAME_ARTICLES_XSELL) . '">' . BUTTON_CANCEL . '</a>'; ?>
                      </td>
                </tr>
              </form>
            </table>
	...
	?>
	<form method="post" action="<?php xtc_href_link(FILENAME_ARTICLES_XSELL, 'sort=1&add_related_article_ID=' . $add_related_article_ID, 'NONSSL'); ?>">
        ...
        $query = "select p.products_id, pd.products_name, pd.products_description, pd.products_url from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "' and p.products_id = ".$xsell_id_pr[$i]."";
                ...
                <tr>
                  <td>&nbsp;</td>
                  <td bgcolor="#CCCCCC"><?php echo '<input type="submit" class="button" onClick="this.blur();" value="' . BUTTON_SAVE . '"/>' . '&nbsp;&nbsp;<a class="button" href="' . xtc_href_link(FILENAME_ARTICLES_XSELL) . '">' . BUTTON_CANCEL . '</a>'; ?></td>
                  <td>&nbsp;</td>
                </tr>
              </table>
            </form>
