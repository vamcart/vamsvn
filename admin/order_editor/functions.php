...
*/
  ...
  function oe_js_zone_list($country, $form, $field, $id, $id2) {
	...
	//kept the vam_ prefix here as the Low Order Fee module uses this function 
  ...
  function oe_get_country_id($country_name) {
    $country_id_query = vam_db_query("select countries_id from " . TABLE_COUNTRIES . " where countries_name = '" . $country_name . "'");
    if (!vam_db_num_rows($country_id_query)) {
      return false;
    }
    else {
      $country_id_row = vam_db_fetch_array($country_id_query);
      return $country_id_row['countries_id'];
    }
  }
  
  function oe_get_country_iso_code_2($country_id) {
    $country_iso_query = vam_db_query("select countries_iso_code_2 from " . TABLE_COUNTRIES . " where countries_id = '" . (int)$country_id . "'");
    if (!vam_db_num_rows($country_iso_query)) {
      return false;
    } else {
      $country_iso_row = vam_db_fetch_array($country_iso_query);
      return $country_iso_row['countries_iso_code_2'];
    }
  }

  function oe_get_country_iso_code_3($country_id) {
    $country_iso_query = vam_db_query("select countries_iso_code_3 from " . TABLE_COUNTRIES . " where countries_id = '" . (int)$country_id . "'");
    if (!vam_db_num_rows($country_iso_query)) {
      return false;
    } else {
      $country_iso_row = vam_db_fetch_array($country_iso_query);
  ...
  function oe_get_zone_id($country_id, $zone_name) {
    $zone_id_query = vam_db_query("select zone_id from " . TABLE_ZONES . " where zone_country_id = '" . (int)$country_id . "' and (zone_name = '" . $zone_name . "' OR zone_code = '" . $zone_name . "')");
    if (!vam_db_num_rows($zone_id_query)) {
      return false;
    }
    else {
      $zone_id_row = vam_db_fetch_array($zone_id_query);
 ...
 //I had to draw up custom functions in order to pass parameters with checkbox fields, maybe radio fields too someday
  function oe_draw_selection_field($name, $type, $value = '', $checked = false, $compare = '', $parameters = '') {
    ...
    if (vam_not_null($value)) $selection .= ' value="' . vam_output_string($value) . '"';
