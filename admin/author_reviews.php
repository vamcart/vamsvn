  ...
  if ($_GET['action']) {
    switch ($_GET['action']) {
      case 'update':
        ...
        vam_db_query("update " . TABLE_AUTHOR_REVIEWS_DESCRIPTION . " set reviews_text = '" . vam_db_input($reviews_text) . "', reviews_answer = '" . vam_db_input($reviews_answer) . "' where reviews_id = '" . vam_db_input($reviews_id) . "'");
      ...
      case 'deleteconfirm':
        ...
        vam_db_query("delete from " . TABLE_AUTHOR_REVIEWS . " where reviews_id = '" . vam_db_input($reviews_id) . "'");
        vam_db_query("delete from " . TABLE_AUTHOR_REVIEWS_DESCRIPTION . " where reviews_id = '" . vam_db_input($reviews_id) . "'");

        vam_redirect(vam_href_link(FILENAME_AUTHOR_REVIEWS, 'page=' . $_GET['page']));
        break;
    }
  }
?>
...
<?php
  if ($_GET['action'] == 'edit') {
    ...
    $article_name_query = vam_db_query("select authors_name from " . TABLE_AUTHORS . " where authors_id = '" . $reviews['authors_id'] . "'");
...
<?php
    ...
    if ($_POST) {
      ...
      $reviews_query = vam_db_query("select r.reviews_id, r.authors_id, r.customers_name, r.customers_avatar, r.date_added, r.last_modified, r.reviews_read, rd.reviews_text, rd.reviews_answer, r.reviews_rating from " . TABLE_AUTHOR_REVIEWS . " r, " . TABLE_AUTHOR_REVIEWS_DESCRIPTION . " rd where r.reviews_id = '" . $_GET['rID'] . "' and r.reviews_id = rd.reviews_id");
      $reviews = vam_db_fetch_array($reviews_query);
      $products_query = vam_db_query("select authors_image from " . TABLE_AUTHORS . " where authors_id = '" . $reviews['authors_id'] . "'");
      $products = vam_db_fetch_array($products_query);
...
<?php
      ...
      if ($_GET['origin']) {
        ...
        $back_url = FILENAME_AUTHOR_REVIEWS;
...
<?php
    ...
    while ($reviews = vam_db_fetch_array($reviews_query)) {
      ...
      if ( (is_object($rInfo)) && ($reviews['reviews_id'] == $rInfo->reviews_id) ) {
        echo '              <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . vam_href_link(FILENAME_AUTHOR_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id . '&action=preview') . '\'">' . "\n";
      } else {
        echo '              <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . vam_href_link(FILENAME_AUTHOR_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $reviews['reviews_id']) . '\'">' . "\n";
      }
?>
                <td class="dataTableContent"><?php echo '<a href="' . vam_href_link(FILENAME_AUTHOR_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $reviews['reviews_id'] . '&action=preview') . '">' . vam_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;' . vam_get_authors_name($reviews['authors_id']); ?></td>
                ...
                <td class="dataTableContent" align="right"><?php if ( (is_object($rInfo)) && ($reviews['reviews_id'] == $rInfo->reviews_id) ) { echo vam_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . vam_href_link(FILENAME_AUTHOR_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $reviews['reviews_id']) . '">' . vam_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
...
<?php
    ...
    switch ($_GET['action']) {
      case 'delete':
        ...
        $contents = array('form' => vam_draw_form('reviews', FILENAME_AUTHOR_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id . '&action=deleteconfirm'));
        $contents[] = array('text' => TEXT_INFO_DELETE_REVIEW_INTRO);
        $contents[] = array('text' => '<br /><b>' . $rInfo->authors_name . '</b>');
        $contents[] = array('align' => 'center', 'text' => '<br /><span class="button"><button type="submit" value="' . BUTTON_DELETE . '">' . vam_image(DIR_WS_IMAGES . 'icons/buttons/delete.png', '', '12', '12') . '&nbsp;' . BUTTON_DELETE . '</button></span> <a class="button" href="' . vam_href_link(FILENAME_AUTHOR_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id) . '"><span>' . vam_image(DIR_WS_IMAGES . 'icons/buttons/cancel.png', '', '12', '12') . '&nbsp;' . BUTTON_CANCEL . '</span></a>');
        break;
