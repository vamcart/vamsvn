    ...
    switch ($_GET['action']) {
      ...
      case 'insert_review': //insert a new item.
        ...
        $avatar = $_POST['customers_avatar_name'];
          ...
          $sql_data_array = array('authors_id'   => vam_db_prepare_input($_POST['authors_id']),
                                  'customers_id'    => '0',
                                  ...
                                  'reviews_read'     => '1' );
          vam_db_perform(TABLE_AUTHOR_REVIEWS, $sql_data_array);
          ...
          vam_db_perform(TABLE_AUTHOR_REVIEWS_DESCRIPTION, $sql_data_array);
          
        }
        vam_redirect(vam_href_link(FILENAME_AUTHOR_REVIEWS));
        break;
...
<!-- body_text //-->
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      ...
      <tr><?php echo vam_draw_form('add_review', FILENAME_AUTHOR_REVIEWS_ADD, vam_get_all_get_params(array('action')) . 'action=insert_review', 'post', 'enctype="multipart/form-data"'); ?>
...
echo vam_draw_products_pull_down_author_review('authors_id','class="select2"'); 
      ...
      <tr>
        <td><table witdh="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td class="main star-rating"><?php for ($i=5; $i>=1; $i--) echo vam_draw_radio_field('reviews_rating', $i, (($i == 5) ? true : false), '', 'class="star-rating" id="star'.$i.'"').'<label for="star'.$i.'" title="'.constant('RATING_STAR_'.$i).'">'.constant('TEXT_STAR_'.$i).'</label>'; ?>
          </tr>
        </table></td>
      </tr>
      <tr>
