...
|     fixed: 2020-04-01 by Valery Shostak https://valery.shostak.ru/sxd     |
|---------------------------------------------------------------------------|
...
class Sypex_Dumper {
  function __construct() { //function Sypex_Dumper() {
    ...
    $this->name = 'Sypex Dumper 2.0.11 [2020-04-01]';
  }
  ...
  function connect($host = null, $port = null, $user = null, $pass = null){
    ...
    if($this->mysqli = mysqli_connect($this->CFG['my_host'], $this->CFG['my_user'], $this->CFG['my_pass'])) { //if(mysql_connect($this->CFG['my_host'] . ($this->CFG['my_host']{0} != ':' ? ":{$this->CFG['my_port']}" : ''),  $this->CFG['my_user'], $this->CFG['my_pass'])) {
      if(V_PHP > 50202) mysqli_set_charset($this->mysqli, 'utf8') or sxd_my_error(); //if(V_PHP > 50202) mysql_set_charset('utf8') or sxd_my_error();
      else mysqli_query($this->mysqli, 'SET NAMES utf8') or sxd_my_error(); //else mysql_query('SET NAMES utf8') or sxd_my_error();
      define('V_MYSQL', sxd_ver2int(mysqli_get_server_info($this->mysqli))); //define('V_MYSQL', sxd_ver2int(mysql_get_server_info()));
    }
    else {
      define('V_MYSQL', 0);
      $this->error = "sxd.actions.tab_connects();alert(" . sxd_esc(mysqli_error($this->mysqli)) . ");"; //$this->error = "sxd.actions.tab_connects();alert(" . sxd_esc(mysql_error()) . ");";
    }
    $this->try = false;
  ...
  function deleteDB($name){
    $r = mysqli_query($this->mysqli, 'DROP DATABASE `' . sxd_esc($name, false) . '`') or sxd_my_error(); //$r = mysql_query('DROP DATABASE `' . sxd_esc($name, false) . '`') or sxd_my_error();
    if($r){
          echo "sxd.clearOpt('db');sxd.addOpt(" . sxd_php2json(array('db' => $this->getDBList())) . ");sxd.combos.services_db.select(0,'-');";
    }
        else
          echo "alert(" . sxd_esc(mysqli_error($this->mysqli)) . ");"; //echo "alert(" . sxd_esc(mysql_error()) . ");";
  }
  function cfg2js($cfg){
    foreach($cfg AS $k => $v){
  ...
  function addDb($req){
        $r = mysqli_query($this->mysqli, 'CREATE DATABASE `' . sxd_esc($req['name'], false) . '`' . (V_MYSQL > 40100 ? "CHARACTER SET {$req['charset']} COLLATE {$req['collate']}" : '')); //$r = mysql_query('CREATE DATABASE `' . sxd_esc($req['name'], false) . '`' . (V_MYSQL > 40100 ? "CHARACTER SET {$req['charset']} COLLATE {$req['collate']}" : ''));
        if($r)
  ...
  function runServices($job) {
    ...
    if(isset($serv[$job['type']])) {
      mysqli_select_db($this->mysqli, $job['db']); //mysql_select_db($job['db']);
      $filter = $object = array();
      $this->createFilters($job['obj'], $filter, $object);
      $r = mysqli_query($this->mysqli, 'SHOW TABLE STATUS') or sxd_my_error(); //$r = mysql_query('SHOW TABLE STATUS') or sxd_my_error();
      if (!$r) return;
      $tables = array();
      while($item = mysqli_fetch_assoc($r)){ //while($item = mysql_fetch_assoc($r)){
      ...
      $r = mysqli_query($this->mysqli, $sql) or sxd_my_error(); //$r = mysql_query($sql) or sxd_my_error();
      if (!$r) return;
      $res = array();
      while($item = mysqli_fetch_row($r)){ //while($item = mysql_fetch_row($r)){
        $res[] = $item;
      }
...
//    @fastcgi_finish_request();
  ...
  function runRestoreJob($continue = false){
    ...
    mysqli_select_db($this->mysqli, $this->JOB['db']); //mysql_select_db($this->JOB['db']);
    ...
    if($this->JOB['strategy'] > 0){
      $r = mysqli_query($this->mysqli, "SHOW TABLES") or sxd_my_error(); //$r = mysql_query("SHOW TABLES") or sxd_my_error();
      while($item = mysqli_fetch_row($r)){ //while($item = mysql_fetch_row($r)){
    ...
    //$enable_index = array();
    if(V_MYSQL > 40014) {
      mysqli_query($this->mysqli, "SET UNIQUE_CHECKS=0"); //mysql_query("SET UNIQUE_CHECKS=0");
      mysqli_query($this->mysqli, "SET FOREIGN_KEY_CHECKS=0"); //mysql_query("SET FOREIGN_KEY_CHECKS=0");
      if(V_MYSQL > 40101) mysqli_query($this->mysqli, "SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO'"); //if(V_MYSQL > 40101) mysql_query("SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO'");
      if(V_MYSQL > 40111) mysqli_query($this->mysqli, "SET SQL_NOTES=0"); //if(V_MYSQL > 40111) mysql_query("SET SQL_NOTES=0");
    }
    $log_sql = false;
    $fields = '';
    $time_old = time();
      ...
      switch($q{0}){
        ...
        case 'I':
          if (preg_match('/^INSERT( INTO `(.+?)`) VALUES/', $q, $m)) {
            ...
            mysqli_query($this->mysqli, "ALTER TABLE `{$tab}` DISABLE KEYS") or sxd_my_error(); //mysql_query("ALTER TABLE `{$tab}` DISABLE KEYS") or sxd_my_error();
...
//            $this->setNames($this->JOB['correct'] == 1 && !empty($this->JOB['charset']) ? $this->JOB['charset'] : empty($m[3]) ? '' : $m[3]);
            ...
            if($m[1] == 'TC') {
              ...
              if($tc && ($this->JOB['strategy'] == 0 || isset($tab_exists[$m[2]]))) {
                mysqli_query($this->mysqli, "{$tc} `{$m[2]}`") or sxd_my_error(); //mysql_query("{$tc} `{$m[2]}`") or sxd_my_error();
              }
            }
            elseif($m[1] == 'TD'){
              $ei = 1;
              ...
              $this->rtl[7] = 0;
              $this->rtl[8] = 0;
              mysqli_query($this->mysqli, "DROP {$types[$m[1]]} IF EXISTS `{$m[2]}`") or sxd_my_error(); //mysql_query("DROP {$types[$m[1]]} IF EXISTS `{$m[2]}`") or sxd_my_error();
      ...
      if($ex) {
        ...
        $this->rtl[1] = time();
        fwrite($this->fh_rtl, implode("\t", $this->rtl));
        if(mysqli_query($this->mysqli, $q)) { //if(mysql_query($q)) {
          if($insert) {
            $c = 1;
          }
        }
        ...
        if($c){
          $i = mysqli_affected_rows($this->mysqli); //$i = mysql_affected_rows();
          $this->rtl[3] = ftell($this->fh_tmp) - $seek;
          $this->rtl[7] += $i;
          $this->rtl[10] += $i;
          fseek($this->fh_rtl, 0);
    ...
    foreach($this->JOB['todo']['TA'] AS $tab){
      ...
      mysqli_query($this->mysqli, "ALTER TABLE `{$tab[1]}` ENABLE KEYS") or sxd_my_error(); //mysql_query("ALTER TABLE `{$tab[1]}` ENABLE KEYS") or sxd_my_error();
      $this->rtl[1] = time();
      $this->rtl[5] = $tab[1];
      fseek($this->fh_rtl, 0);
      fwrite($this->fh_rtl, implode("\t", $this->rtl));
  ...
  function addBackupJob($job) {
    ...
    foreach($queries AS $query){
      ...
      if($t == 'TA' && (!empty($object['TC']) || !empty($filter['TC']))) {}
      elseif(empty($object[$t]) && empty($filter[$t])) continue;
      $r = mysqli_query($this->mysqli, 'SHOW ' . $query[0]) or sxd_my_error(); //$r = mysql_query('SHOW ' . $query[0]) or sxd_my_error();
      if (!$r) continue;
      $todo[$t] = array();
      $header[$t] = array();
  ...
  function runBackupJob($continue = false){
    ...
    mysqli_query($this->mysqli, "SET SQL_QUOTE_SHOW_CREATE = 1"); //mysql_query("SET SQL_QUOTE_SHOW_CREATE = 1");
    $types = array('VI' => 'View', 'PR' => 'Procedure', 'FU' => 'Function', 'TR' => 'Trigger', 'EV' => 'Event');
    $fcache = '';
    $writes = 0;

    if(V_MYSQL > 40101) mysqli_query($this->mysqli, "SET SESSION character_set_results = '" . ($this->JOB['charset'] ? $this->JOB['charset'] : 'binary') ."'") or sxd_my_error(); //if(V_MYSQL > 40101) mysql_query("SET SESSION character_set_results = '" . ($this->JOB['charset'] ? $this->JOB['charset'] : 'binary') ."'") or sxd_my_error();
    $time_old = time();
    $exit_time = $time_old + $this->CFG['time_web'] - 1;
        ...
        switch($n[0]){
          ...
          case 'TA':
            ...
            if ($n[0] == 'TC' || $this->rtl[7] == 0){
              ...
              $r = mysqli_query($this->mysqli, "SHOW CREATE TABLE `{$n[1]}`") or sxd_my_error(); //$r = mysql_query("SHOW CREATE TABLE `{$n[1]}`") or sxd_my_error();
              $item = mysqli_fetch_assoc($r); //$item = mysql_fetch_assoc($r);
                $fcache .= "#\tTC`{$n[1]}`{$n[2]}\t;\n{$item['Create Table']}\t;\n";
            ...
            $notNum = array();
            $r = mysqli_query($this->mysqli, "SHOW COLUMNS FROM `{$n[1]}`") or sxd_my_error(); //$r = mysql_query("SHOW COLUMNS FROM `{$n[1]}`") or sxd_my_error();
                  $fields = 0;
                  while($col = mysqli_fetch_array($r)) { //while($col = mysql_fetch_array($r)) {
                  ...
                  $z = 0;
                  // Достаем данные
                  $r = mysqli_query($this->mysqli, "SELECT {$no_cache}* FROM `{$n[1]}`{$from}", MYSQLI_USE_RESULT); //$r = mysql_unbuffered_query("SELECT {$no_cache}* FROM `{$n[1]}`{$from}");
                  while($row = mysqli_fetch_row($r)) { //while($row = mysql_fetch_row($r)) {
              ...
              for($k = 0; $k < $fields; $k++){
                ...
                elseif($notNum[$k]) {$row[$k] =  '\'' . mysqli_real_escape_string($this->mysqli, $row[$k]) . '\'';} //elseif($notNum[$k]) {$row[$k] =  '\'' . mysql_real_escape_string($row[$k]) . '\'';} // TODO: Потестить скорость эскэйпинга строк
              }
              $fcache .= '(' . implode(',', $row) . "),\n";
              $this->rtl[7]++;
              $this->rtl[10]++;
            ...
            unset($row);
            mysqli_free_result($r); //mysql_free_result($r);
          ...
          default:
            if(V_MYSQL < 50121 && $n[0] == 'TR'){
              ...
              $r = mysqli_query($this->mysqli, "SELECT * FROM `INFORMATION_SCHEMA`.`TRIGGERS` WHERE `TRIGGER_SCHEMA` = '{$this->JOB['db']}' AND `TRIGGER_NAME` = '{$n[1]}'") or sxd_my_error(); //$r = mysql_query("SELECT * FROM `INFORMATION_SCHEMA`.`TRIGGERS` WHERE `TRIGGER_SCHEMA` = '{$this->JOB['db']}' AND `TRIGGER_NAME` = '{$n[1]}'") or sxd_my_error();
              $item = mysqli_fetch_assoc($r); //$item = mysql_fetch_assoc($r);
              ...
              $r = mysqli_query($this->mysqli, "SHOW CREATE {$types[$n[0]]} `{$n[1]}`") or sxd_my_error(); //$r = mysql_query("SHOW CREATE {$types[$n[0]]} `{$n[1]}`") or sxd_my_error();
              $item = mysqli_fetch_assoc($r); //$item = mysql_fetch_assoc($r);
  ...
  function setNames($collation){
    ...
    if($this->rtl[6] != $collation) {
      mysqli_query($this->mysqli, 'SET NAMES \'' . preg_replace('/^(\w+?)_/', '\\1\' COLLATE \'\\1_', $collation) . '\'') or sxd_my_error(); //mysql_query('SET NAMES \'' . preg_replace('/^(\w+?)_/', '\\1\' COLLATE \'\\1_', $collation) . '\'') or sxd_my_error();
  ...
  function getDBList(){
    ...
    if ($this->CFG['my_db']) {
      ...
      $result = mysqli_query($this->mysqli, "SHOW DATABASES") or sxd_my_error(); //$result = mysql_query("SHOW DATABASES") or sxd_my_error();
        while($item = mysqli_fetch_row($result)){ //while($item = mysql_fetch_row($result)){
    ...
    if(V_MYSQL < 50000){
      foreach($items AS $item){
          $tables = mysqli_query($this->mysqli, "SHOW TABLES FROM `{$item}`") or sxd_my_error(); //$tables = mysql_query("SHOW TABLES FROM `{$item}`") or sxd_my_error();
          if ($tables) {
              $tabs = mysqli_num_rows($tables); //$tabs = mysql_num_rows($tables);
              $dbs[$item] = "{$item} ({$tabs})";
            }
      }
    }
      ...
      $result = mysqli_query($this->mysqli, "SELECT `table_schema`, COUNT(*) FROM `information_schema`.`tables` {$where} GROUP BY `table_schema`") or sxd_my_error(); //$result = mysql_query("SELECT `table_schema`, COUNT(*) FROM `information_schema`.`tables` {$where} GROUP BY `table_schema`") or sxd_my_error();
      while($item = mysqli_fetch_row($result)){ //while($item = mysql_fetch_row($result)){
          if($item[0] == 'information_schema' || $item[0] == 'mysql' || $item[0] == 'performance_schema') continue;
  ...
  function getCharsetList(){
    ...
    if(V_MYSQL > 40101) {
      ...
      if(!empty($this->CFG['charsets'])){
        ...
        $r = mysqli_query($this->mysqli, "SHOW CHARACTER SET") or sxd_my_error(); //$r = mysql_query("SHOW CHARACTER SET") or sxd_my_error();
        if ($r) {
          while($item = mysqli_fetch_assoc($r)){ //while($item = mysql_fetch_assoc($r)){
  ...
  function getCollationList(){
    ...
    if(V_MYSQL > 40101) {
      ...
      if(!empty($this->CFG['charsets'])){
        ...
        $r = mysqli_query($this->mysqli, "SHOW COLLATION") or sxd_my_error(); //$r = mysql_query("SHOW COLLATION") or sxd_my_error();
        if ($r) {
          while($item = mysqli_fetch_assoc($r)){ //while($item = mysql_fetch_assoc($r)){
  ...
  function getObjects($tree, $db_name){
    mysqli_select_db($this->mysqli, $db_name); //mysql_select_db($db_name);
    // Достаем таблицы
    $r = mysqli_query($this->mysqli, 'SHOW TABLE STATUS'); //$r = mysql_query('SHOW TABLE STATUS');
    ...
    if($r){
      ...
      if (V_MYSQL > 50014 && $tree != 'services') {
        ...
        for($i = 0, $l = count($shows); $i < $l; $i++){
          $r = mysqli_query($this->mysqli, 'SHOW ' . $shows[$i]); //$r = mysql_query('SHOW ' . $shows[$i]);
          if($r && mysqli_num_rows($r) > 0) { //if($r && mysql_num_rows($r) > 0) {
...
function sxd_my_error(){
  trigger_error(mysqli_error($this->mysqli), E_USER_ERROR); //trigger_error(mysql_error(), E_USER_ERROR);
}
